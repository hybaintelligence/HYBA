...
class TestMathematicalFoundations:
    ...
    def test_hilbert_space_dimension(self):
        ...
        for n in range(1, 6):
            ...
            assert (
                qc.hilbert_dimension == 2**n
            ), f"Hilbert dim {qc.hilbert_dimension} != 2^{n}"
    ...
    def test_basis_state_initialization(self):
        ...
        assert all(
            abs(state.amplitudes[i]) < 1e-10 for i in range(1, len(state.amplitudes))
...
class TestGateMathematics:
    ...
    def test_phase_gate_unitarity(self):
        ...
        for theta in [0, np.pi / 4, np.pi / 2, np.pi, 2 * np.pi]:
            ...
            assert (
                verification.passed
            ), f"Phase gate (θ={theta}) not unitary: {verification}"
    ...
    def test_gate_determinant_magnitude(self):
        ...
        for gate in gates:
            ...
            assert (
                abs(abs(det) - 1.0) < 1e-10
            ), f"|det({gate.name})| = {abs(det)} != 1.0"
...
class TestStateEvolution:
    ...
    def test_two_qubit_gate_preserves_normalization(self):
        ...
        assert (
            abs(norm - 1.0) < 1e-10
        ), f"CNOT did not preserve normalization: norm = {norm}"
    ...
    def test_hadamard_creates_superposition(self):
        ...
        assert (
            abs(abs(state.amplitudes[0]) - 1 / sqrt(2)) < 1e-10
        ), f"Amplitude |0⟩ = {abs(state.amplitudes[0])} != 1/√2"
        assert (
            abs(abs(state.amplitudes[1]) - 1 / sqrt(2)) < 1e-10
    ...
    def test_pauli_x_flips_basis_state(self):
        ...
        assert (
            abs(abs(state.amplitudes[0]) - 0.0) < 1e-10
        ), "X|0⟩ should have no |0⟩ component"
        ...
        assert (
            abs(abs(state.amplitudes[1]) - 0.0) < 1e-10
        ), "X|1⟩ should have no |1⟩ component"
    ...
    def test_cnot_creates_entanglement(self):
        ...
        assert (
            abs(abs(state.amplitudes[3]) - 1 / sqrt(2)) < 1e-10
        ), f"Bell state |11⟩ amplitude = {abs(state.amplitudes[3])} != 1/√2"
        assert (
            abs(abs(state.amplitudes[1])) < 1e-10
        ), "Bell state should have no |01⟩ component"
        assert (
            abs(abs(state.amplitudes[2])) < 1e-10
        ), "Bell state should have no |10⟩ component"
...
class TestParallelGates:
    ...
    def test_parallel_gates_on_disjoint_qubits(self):
        ...
        assert np.allclose(
            state_parallel.amplitudes, state_sequential.amplitudes, atol=1e-10
    ...
    def test_parallel_gates_preserve_normalization(self):
        ...
        assert (
            abs(norm - 1.0) < 1e-10
        ), f"Parallel gates did not preserve normalization: norm = {norm}"
    ...
    def test_parallel_efficiency_is_perfect(self):
        ...
        parallel_op = [
            op for op in audit_log if op["operation"] == "apply_parallel_gates"
        ]
        assert len(parallel_op) > 0, "Parallel gate operation not logged"
        assert (
            parallel_op[-1]["metadata"]["parallel_efficiency"] == 1.0
...
class TestMeasurement:
    ...
    def test_born_rule_probabilities_sum_to_one(self):
        ...
        ), f"Probabilities sum to {total_prob} != 1.0"
    ...
    def test_measurement_collapses_state(self):
        ...
        assert (
            non_zero_count == 1
        ), f"Collapsed state has {non_zero_count} non-zero amplitudes, should be 1"
    ...
    def test_measurement_outcome_matches_probability(self):
        ...
        assert (
            abs(prob_0 - expected_prob) < 3 * std_dev
...
class TestPulviniCompression:
    ...
    def test_pulvini_compression_reduces_dimension(self):
        ...
        if state.compression_metadata:
            assert (
                state.compression_metadata["folded_dimension"]
                < state.compression_metadata["original_dimension"]
...
class TestPhiOptimization:
    ...
    def test_phi_optimization_identifies_parallel_gates(self):
        ...
        assert (
            len(optimized[0]) == 3
        ), f"Expected 3 gates in group, got {len(optimized[0])}"
    ...
    def test_phi_optimization_reduces_circuit_depth(self):
        ...
        assert np.allclose(
            state_no_opt.amplitudes, state_opt.amplitudes, atol=1e-10
...
class TestMathematicalInvariants:
    ...
    def test_trace_preservation_invariant(self):
        ...
        assert (
            abs(trace_before - trace_after) < 1e-10
...
class TestAuditSystem:
    ...
    def test_audit_comprehensive(self):
        ...
        assert hasattr(
            audit, "parallel_efficiency"
        ), "Audit missing parallel efficiency"
        assert hasattr(
            audit, "mathematical_invariants"
        ), "Audit missing invariants list"
...
class TestNumericalPrecision:
    ...
    def test_small_amplitudes_preserved(self):
        ...
        assert np.any(
            np.abs(state_after.amplitudes) > eps / 10
