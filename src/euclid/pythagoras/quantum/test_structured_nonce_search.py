...
class TestEmpiricalEvidence:
    ...
    def test_load_empirical_evidence_from_json(self):
        ...
        try:
            ...
            assert evidence.phi_resonance_rate == 0.95652174, \
                f"Expected 0.95652174, got {evidence.phi_resonance_rate}"
            assert evidence.z_score == 7.584309, \
                f"Expected 7.584309, got {evidence.z_score}"
            assert evidence.p_value == "4.20e-14", \
                f"Expected 4.20e-14, got {evidence.p_value}"
            assert evidence.mean_resonance_strength == 0.474603, \
    ...
    def test_default_empirical_evidence(self):
        ...
        assert evidence.unsearched_gaps == 60, \
    ...
    def test_empirical_evidence_statistical_significance(self):
        ...
        assert evidence.phi_resonance_rate > 0.5, \
...
class TestPhiResonanceCalculation:
    ...
    def test_phi_resonance_perfect_alignment(self):
        ...
        assert isclose(resonance, 1.0, rel_tol=1e-6), f"Perfect alignment should have resonance≈1.0, got {resonance}"
        # Distance should be very small (due to integer truncation of phi_15)
        assert distance < 1.0, f"Perfect alignment should have small distance, got {distance}"
    ...
    def test_phi_resonance_no_alignment(self):
        ...
        assert resonance < 0.6, f"No alignment should have low resonance, got {resonance}"
...
class TestStructureScoring:
    ...
    def test_structure_score_incorporates_resonance(self):
        ...
        score_perfect = search.compute_structure_score(int(search.phi_15), resonance_perfect)
        
        # No resonance
        resonance_none, _ = search.compute_phi_resonance(int(search.phi_15) + int(search.phi_15/2))
        score_none = search.compute_structure_score(int(search.phi_15) + int(search.phi_15/2), resonance_none)
        
        assert score_perfect > score_none, \
    ...
    def test_structure_score_incorporates_empirical_prior(self):
        ...
        assert isclose(score_with_prior, expected_weighting, rel_tol=0.1), \
    ...
    def test_structure_score_temporal_modulation(self):
        ...
        score_no_height = search.compute_structure_score(1000, resonance, block_height=None)
        
        # Score with block height
        score_with_height = search.compute_structure_score(1000, resonance, block_height=800000)
        
        # Scores should differ due to temporal modulation
        assert not isclose(score_no_height, score_with_height, rel_tol=1e-10), \
...
class TestAmplitudeAmplification:
    ...
    def test_amplitude_amplification_boosts_high_scores(self):
        ...
        candidates = [
            NonceCandidate(nonce=100, resonance_strength=0.9, phi_15_distance=10, structure_score=0.9),
            NonceCandidate(nonce=200, resonance_strength=0.5, phi_15_distance=100, structure_score=0.5),
            NonceCandidate(nonce=300, resonance_strength=0.1, phi_15_distance=1000, structure_score=0.1),
        ]
        ...
        assert high_score_after >= high_score_before * 0.7, \
    ...
    def test_amplitude_amplification_preserves_ordering(self):
        ...
        candidates = [
            NonceCandidate(nonce=100, resonance_strength=0.9, phi_15_distance=10, structure_score=0.9),
            NonceCandidate(nonce=200, resonance_strength=0.7, phi_15_distance=50, structure_score=0.7),
            NonceCandidate(nonce=300, resonance_strength=0.5, phi_15_distance=100, structure_score=0.5),
        ]
        
        amplified = search.amplitude_amplification(candidates, iterations=3)
        
        # Ordering should be preserved (highest score first)
        assert amplified[0].structure_score >= amplified[1].structure_score, \
            "Ordering not preserved after amplification"
        assert amplified[1].structure_score >= amplified[2].structure_score, \
            "Ordering not preserved after amplification"
...
class TestCandidateGeneration:
    ...
    def test_candidate_generation_phi_guided_spacing(self):
        ...
        spacings = [candidates[i+1].nonce - candidates[i].nonce for i in range(len(candidates)-1)]
    ...
    def test_candidate_generation_structure_score(self):
        ...
        for i in range(10):
            candidate = search.generate_candidate(i)
            assert 0.0 <= candidate.structure_score <= 1.0, \
    ...
    def test_candidate_generation_resonance_computed(self):
        ...
        for i in range(10):
            candidate = search.generate_candidate(i)
            assert 0.0 <= candidate.resonance_strength <= 1.0, \
                f"Resonance strength {candidate.resonance_strength} not in [0, 1]"
            assert candidate.phi_15_distance >= 0, \
...
class TestStructuredSearch:
    ...
    def test_search_max_attempts_respected(self):
        ...
        assert result.attempts <= 50, f"Attempts {result.attempts} should not exceed max_attempts"
    ...
    def test_search_without_amplification(self):
        ...
        result = search.search(target_function, max_attempts=100, use_amplification=False)
        
        assert result.found_nonce is not None, "Search should find nonce without amplification"
...
class TestBenchmarking:
    ...
    def test_benchmark_structured_vs_random(self):
        ...
        ratio = benchmark["structured_mean_attempts"] / benchmark["random_mean_attempts"]
        assert ratio <= 2.0, \
...
class TestMathematicalCorrectness:
    ...
    def test_phi_constant_accuracy(self):
        ...
        assert isclose(search.phi_15, expected_phi_15, rel_tol=1e-10), \
