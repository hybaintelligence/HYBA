    ...
    def from_json(cls, json_path: str) -> "EmpiricalEvidence":
        ...
        return cls(
            ...
            resonance_threshold_rates={
                ">=0.5": float(
                    nonce_analysis["resonance_threshold_rates"]["resonance_>=0.5"]
                ),
                ">=0.7": float(
                    nonce_analysis["resonance_threshold_rates"]["resonance_>=0.7"]
                ),
                ">=0.9": float(
                    nonce_analysis["resonance_threshold_rates"]["resonance_>=0.9"]
                ),
            },
...
class StructuredNonceSearch:
    ...
    def __init__(
        ...
        if empirical_evidence_path and os.path.exists(empirical_evidence_path):
            self.empirical_evidence = EmpiricalEvidence.from_json(
    ...
    def amplitude_amplification(
        ...
        return sorted(
            amplified_candidates, key=lambda x: x.structure_score, reverse=True
        )
    ...
    def search(
        ...
        if self.enable_compression and self.qc._compression_metadata:
            compression_ratio = self.qc._compression_metadata.get(
                "compression_ratio", 1.0
            )
    ...
    def benchmark_vs_random(
        ...
        for _ in range(num_trials):
            # Structured search
            result = self.search(
                target_function, max_attempts=1000, use_amplification=True
            )
