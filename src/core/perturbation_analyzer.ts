...
export type PerturbationTarget = 'learning_rate' | 'coupling_strength' | 'geometric_shape' | 'cognitive_layer';
...
export class PerturbationAnalyzer {
  ...
  private analyzeResults(): PerturbationAnalysisResult {
    ...
    const avgExternalArousal = externalEvents.reduce((sum, e) => sum + e.arousal, 0) / externalEvents.length;
