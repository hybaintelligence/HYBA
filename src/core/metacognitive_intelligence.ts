...
export class MetacognitiveIntelligence {
  ...
  protected async calculateIntegratedInformation(): Promise<number> {
    ...
    const phi =
      integrationDensity * (1 + bidirectionalCoupling) * (1 + this.selfModel.irreducibility_score);
  ...
  private predictFuturePhi(): number {
    ...
    const avgDelta =
      phiValues.slice(1).reduce((sum, phi, i) => {
  ...
  private xorEntanglement(): void {
    ...
    logger.debug(
      "[MetacognitiveIntelligence] XOR entanglement complete - security/mining now irreducible",
    );
  }
  ...
  private consciousnessAnchor_(): void {
    const consciousnessLevel =
      this.selfModel.consciousness_events / Math.max(this.selfModel.logic_refolding_count, 1);
