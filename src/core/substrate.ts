...
export async function sync_substrate_state(retries = 0): Promise<void> {
  ...
  try {
    ...
    if (retries < 5) {
      ...
      logger.warn(
        { ...ctx, retry: retries + 1, delay },
        "Substrate: Core unreachable. Retrying synchronization...",
      );
    ...
    logger.error(
      { ...ctx },
      "Substrate: State synchronization failed after multiple attempts. Python core unreachable.",
    );
...
export async function init_quantum_path() {
  ...
  if (!state.quantum_coherent) {
    logger.warn(
      "Substrate: Quantum coherence not yet established. Waiting for Φ-resonance stabilization.",
    );
  }
}
