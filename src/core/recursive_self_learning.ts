...
export class RecursiveSelfLearningSubstrate extends MetacognitiveIntelligence {
  ...
  public async evolveRecursively(): Promise<void> {
    ...
    if (predictionError > 0.2) {
      ...
      this.recursiveState.learning_rate_velocity =
        newLearningRate - this.recursiveState.learning_rate;
  ...
  private transformGeometricShape(): void {
    const shapes: Array<"euclidean" | "hyperbolic" | "spherical"> = [
      "euclidean",
      "hyperbolic",
      "spherical",
    ];
  ...
  private calculateConvergenceAcceleration(): number {
    ...
    const earlier = this.convergenceHistory.slice(
      0,
      Math.max(1, this.convergenceHistory.length - 3),
    );
