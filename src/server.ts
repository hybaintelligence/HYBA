...
const circuitState: CircuitState = { failures: 0, lastFailureTime: 0, isOpen: false, halfOpenAttempted: false };
...
function recordProxyFailure(): void {
  ...
  if (circuitState.failures >= CIRCUIT_THRESHOLD && !circuitState.isOpen) {
    ...
    logger.error({ failures: circuitState.failures, threshold: CIRCUIT_THRESHOLD, resetTimeMs: CIRCUIT_RESET_MS, backendUrl: CONFIG.backendUrl.toString(), timestamp: new Date().toISOString() }, "🚨 CIRCUIT BREAKER TRIPPED — Backend proxy circuit opened");
  }
}
...
function recordHealthCheckFailure(): void {
  ...
  if (metrics.healthCheckFailures >= HEALTH_CHECK_FAILURE_THRESHOLD && timeSinceFirstFailure <= HEALTH_CHECK_FAILURE_WINDOW_MS) {
    logger.error({ healthCheckFailures: metrics.healthCheckFailures, threshold: HEALTH_CHECK_FAILURE_THRESHOLD, windowMs: HEALTH_CHECK_FAILURE_WINDOW_MS, backendUrl: CONFIG.backendUrl.toString(), timestamp: new Date().toISOString() }, "🚨 HEALTH CHECK FAILURE THRESHOLD EXCEEDED — Backend consistently unreachable");
  }
}

function recordHealthCheckSuccess(): void {
  if (metrics.healthCheckFailures > 0) {
    logger.info({ previousFailures: metrics.healthCheckFailures, timestamp: new Date().toISOString() }, "Health check recovered — backend reachable again");
  }
...
async function autoConnectViaBTC(): Promise<void> {
  ...
  try {
    ...
    logger.warn({ err: error instanceof Error ? error.message : String(error) }, "Auto-connect to mining pool threw");
  }
}
...
async function isBackendReachable(): Promise<boolean> {
  ...
  try {
    const response = await fetch(new URL("/api/health/readiness", CONFIG.backendUrl), { signal: controller.signal });
    return response.ok;
  } catch {
    return false;
  } finally {
...
function spawnBackend(): void {
  ...
  backendProcess = spawn(python, ["-m", "uvicorn", "hyba_genesis_api.main:app", "--host", backendHost, "--port", backendPort, "--log-level", process.env.LOG_LEVEL === "debug" ? "debug" : "warning"], {
    cwd: process.cwd(),
...
async function proxyToBackend(req: Request, res: Response): Promise<void> {
  if (isCircuitOpen()) {
    noStore(res);
    res.status(503).json({ error: "circuit_breaker_open", message: "Backend circuit breaker is open — too many recent failures", retryAfterMs: CIRCUIT_RESET_MS });
    return;
  }
  ...
  try {
    ...
    const response = await fetch(target, { method: req.method.toUpperCase(), headers, body: bodyBuffer ? new Uint8Array(bodyBuffer) : undefined, signal: controller.signal });
    ...
    recordProxyFailure();
    logger.error({ err: error instanceof Error ? error.message : "Unknown proxy error", path: req.originalUrl, requestId }, "Backend proxy request failed");
    noStore(res);
    res.status(503).json({ error: "backend_unavailable", message: "HYBA backend is not reachable", path: req.originalUrl, requestId });
  } finally {
    clearTimeout(timeout);
  }
}
...
export function registerSecuritySwarmRoutes(app: express.Application, swarm = securitySwarms): void {
  app.get("/api/security/status", async (req: Request, res: Response) => {
    ...
    res.json({
      ...
      defense_systems: {
        stabilizer_monitor: { ...sample, confidence_threshold: status.confidence_threshold, syndrome_width: status.syndrome_width, operating_mode: status.operating_mode, syndrome_check_stride: status.syndrome_check_stride, check_frequency: status.check_frequency, syndrome_rotation_index: status.syndrome_rotation_index, pool_permutation_checksum: status.pool_permutation_checksum, sanitized: status.sanitized, metacognitive: { current_state: status.operating_mode, predicted_state: status.anomaly_detected ? "integrity_response_active" : "nominal", confidence: status.last_confidence, self_awareness: status.last_confidence } },
        preallocated_ancilla_trap_pool: { agents_total: status.agents_total, agents_active: status.agents_active, logical_agents: status.logical_agents, reserved_ancillas: status.reserved_ancillas, active_ancillas: status.active_ancillas, max_ancilla_pool: status.max_ancilla_pool, reserved_traps: status.reserved_traps, active_traps: status.active_traps, disturbed_traps: status.disturbed_traps, retired_traps: status.retired_traps },
      },
      recent_threats: sample.anomaly_detected ? [{ type: sample.cause, syndrome_weight: sample.syndrome_weight, trap_disturbances: sample.trap_disturbances, detected_at: new Date().toISOString() }] : [],
    });
  });
...
async function startServer(): Promise<void> {
  ...
  app.use((req: Request, _res: Response, next: NextFunction) => { metrics.requestsTotal += 1; const pathKey = req.method + " " + req.path; metrics.requestsByPath.set(pathKey, (metrics.requestsByPath.get(pathKey) || 0) + 1); next(); });
  app.use(rateLimit({ windowMs: CONFIG.rateLimitWindowMs, max: CONFIG.rateLimitMax, message: { error: "too_many_requests", message: "Too many requests, please try again later." }, standardHeaders: true, legacyHeaders: false }));

  app.get("/", async (_req: Request, res: Response, next: NextFunction) => { if (!CONFIG.isProduction) return next(); noStore(res); res.json({ status: "online", service: "HYBA Secure Bridge", version: "2.1.0", backendReachable: await isBackendReachable(), timestamp: new Date().toISOString() }); });
  app.get("/bridge/health", async (_req: Request, res: Response) => { const reachable = await isBackendReachable(); if (!reachable) recordHealthCheckFailure(); else recordHealthCheckSuccess(); noStore(res); res.status(reachable ? 200 : 503).json({ status: reachable ? "ok" : "degraded", service: "HYBA Secure Bridge", version: "2.1.0", backendReachable: reachable, timestamp: new Date().toISOString() }); });
  app.get("/bridge/internal/health", requireInternalAccess, async (_req: Request, res: Response) => { const reachable = await isBackendReachable(); const uptimeSeconds = Math.floor((Date.now() - metrics.startTime) / 1000); noStore(res); res.status(reachable ? 200 : 503).json({ status: reachable ? "ok" : "degraded", service: "HYBA Secure Bridge", version: "2.1.0", backend: CONFIG.backendUrl.toString(), backendReachable: reachable, circuitBreakerOpen: circuitState.isOpen, circuitFailures: circuitState.failures, uptimeSeconds, metrics: { requestsTotal: metrics.requestsTotal, proxyErrors: metrics.proxyErrors, circuitBreakerTrips: metrics.circuitBreakerTrips, topPaths: Array.from(metrics.requestsByPath.entries()).sort((a, b) => b[1] - a[1]).slice(0, 10) }, timestamp: new Date().toISOString() }); });
  ...
  app.get("/api/intelligence/telemetry", requireInternalAccess, async (_req: Request, res: Response) => { noStore(res); res.json(intelligenceService.getTelemetry()); });
  app.get("/api/intelligence/status", requireInternalAccess, async (_req: Request, res: Response) => { noStore(res); res.json({ active: intelligenceService.isActive(), phi: intelligenceService.getPhi(), current_goal: intelligenceService.getCurrentGoal(), uptime_ms: Date.now() - metrics.startTime }); });
  app.get("/api/intelligence/hebbian-stats", requireInternalAccess, async (_req: Request, res: Response) => { noStore(res); res.json(intelligenceService.getHebbianStats()); });
  app.post("/api/intelligence/simulate-disturbance", requireInternalAccess, async (req: Request, res: Response) => { const { syndrome } = req.body as { syndrome?: number }; if (typeof syndrome !== "number") { res.status(400).json({ error: "invalid_request", message: "syndrome must be a number" }); return; } intelligenceService.simulateDisturbance(syndrome); noStore(res); res.json({ status: "disturbance_applied", telemetry_source: "operator_authorised_internal_route" }); });
  app.post("/api/intelligence/reset", requireInternalAccess, async (_req: Request, res: Response) => { intelligenceService.reset(); noStore(res); res.json({ status: "reset_complete" }); });
  app.post("/api/intelligence/start", requireInternalAccess, async (_req: Request, res: Response) => { intelligenceService.start(); noStore(res); res.json({ status: "intelligence_started" }); });
  app.post("/api/intelligence/stop", requireInternalAccess, async (_req: Request, res: Response) => { intelligenceService.stop(); noStore(res); res.json({ status: "intelligence_stopped" }); });
  ...
  if (!CONFIG.isProduction) {
    ...
    const viteInlineConfig: InlineConfig = { configFile: false, root: projectRoot, cacheDir: getViteCacheDir(projectRoot), plugins: [react(), tailwindcss()], resolve: { alias: { "@": projectRoot } }, server: { middlewareMode: true, hmr: process.env.DISABLE_HMR !== "true", watch: process.env.DISABLE_HMR === "true" ? null : {}, fs: { strict: false, allow: [projectRoot] } }, appType: "spa", optimizeDeps: { include: ["react", "react-dom", "react-dom/client", "lucide-react", "recharts", "d3", "motion", "firebase/app", "firebase/firestore", "firebase/auth"] } };
  ...
  server.on("error", (error: NodeJS.ErrnoException) => { if (error.code === "EADDRINUSE") { logger.error({ port: CONFIG.port }, `Port ${CONFIG.port} is already in use`); process.exit(1); } logger.error({ err: error }, "HTTP server failed"); process.exit(1); });
  server.listen(CONFIG.port, CONFIG.host, () => { void autoConnectViaBTC(); logger.info({ port: CONFIG.port, host: CONFIG.host, backendUrl: CONFIG.backendUrl.toString() }, "HYBA Secure Bridge listening"); });
}

function installShutdownHandlers(server: Server): void {
  const shutdown = (signal: string) => { logger.info({ signal }, "Graceful shutdown initiated — draining connections"); server.close(() => logger.info("HTTP server closed — all connections drained")); if (backendProcess && !backendProcess.killed) { backendProcess.kill("SIGTERM"); logger.info("Backend process terminated"); } setTimeout(() => { logger.info("Shutdown complete — exiting"); process.exit(0); }, 5000).unref(); };
...
if (process.env.NODE_ENV !== "test") {
  startServer().catch((error: unknown) => { logger.fatal({ err: error instanceof Error ? error.message : String(error) }, "Critical server failure on startup"); process.exit(1); });
}
