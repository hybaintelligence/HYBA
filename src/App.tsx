...
import {
  ...
  MetricExplainerCard,
  EvidenceBoundAnswer,
} from "./components/IntelligenceTranslator";
import {
  DecisionCockpit,
  ProofExplainer,
  UseCaseStudio as ImportedUseCaseStudio,
...
function AppContent() {
  ...
  const handleExportAuditMemo = async (format: "json" | "pdf") => {
    ...
    setAuthFeedback({
      text: `Signed audit memo exported as ${format.toUpperCase()}.`,
      error: false,
    });
  };
          ...
          {currentView === "admin" ? (
              ...
              fallback={
                <div className="rounded-lg border border-red-200 bg-red-50 p-8 text-center">
                  ...
                  <p className="text-red-700 text-sm mt-2">
                    This surface is available only to internal treasury personnel.
                  </p>
                </div>
              }
            >
            ...
            <>
              ...
              <DecisionCockpit
                ...
                previousStability={previousReasoningStability}
                veracity={reasoningVeracity}
              ...
              <section className="grid grid-cols-1 gap-4 lg:grid-cols-3">
                ...
                />
                <MetricExplainerCard
                  metricKey="evidence_seal"
                  value={`Seal: ${extraordinarySeal}`}
                />
              </section>
              ...
              <section className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                ...
                >
                  ...
                  <MetricRow
                    label="Hashrate (EH/s)"
                    value={fmtNum(systemMetrics.currentHashrate)}
                  />
                  <MetricRow
                ...
                <div className="space-y-4">
                  <MetricExplainerCard
                    metric="substrate_coherence"
                    value={fmtPct(health?.quantumCoherence)}
                  />
                  <Panel
                  ...
                  >
                    ...
                    <select
                      value={phiTier}
                      onChange={(e) =>
                        handlePowerScaleChange(powerScale, parseInt(e.target.value, 10))
                      }
                ...
                <div className="space-y-4">
                  <EvidenceBoundAnswer
                    ...
                    claimBoundary={fmtText(
                      extraordinaryEvidence?.claim_boundary ||
                        "Advisory intelligence; not autonomous execution",
                    )}
                    invariantStatus={
                      extraordinaryEvidence?.all_invariants_passed
                        ? "Passed"
...
function StrategicDecisionWorkbench({
  ...
  const stability = clamp(
    coherence * 55 + resonance * 45 - latencyStress * 0.18 - demandShock * 0.1,
  );
  ...
  const agents = [
    {
      name: "Manifold patterns",
      role: "Pattern field",
      signal: "Detects topology shift under load",
      weight: 34,
    },
    {
      name: "PULVINI memory",
      role: "Context memory",
      signal: "Compares current state with sealed prior runs",
      weight: 29,
    },
    {
      name: "Salamander regeneration",
      role: "Recovery planner",
      signal: "Produces rollback and safe-remediation paths",
      weight: 22,
    },
    {
      name: "Governance guard",
      role: "Approval boundary",
      signal: "Keeps output proposal-only until signed",
      weight: 15,
    },
  ];
  const dag = [
    ...
    [
      "Recommendation",
      stability > 65
        ? "Proceed with monitored intervention"
        : "Defer action and request human triage",
    ],
  ...
  return (
    <section
      className="rounded-[2rem] border border-slate-200 bg-white/95 p-6 shadow-xl"
      data-testid="strategic-decision-workbench"
    >
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="eyebrow">
            <SlidersHorizontal className="h-3.5 w-3.5" /> Counterfactual Sandbox
          </p>
          <h2 className="mt-2 text-2xl font-black text-slate-950">
            Stress-test the recommendation before anyone acts.
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Manipulate operational assumptions and HYBA recalculates the predicted outcome, causal
            chain, collaborating sub-intelligences, and cost of certainty from live telemetry (
            {telemetrySource}).
          </p>
        </div>
        <span className="rounded-full border border-amber-200 bg-amber-50 px-3 py-1 text-xs font-bold text-amber-800">
          Simulation only · proposal boundary
        </span>
      </div>
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
        <div className="space-y-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
          <ScenarioSlider
            label="What if latency spikes?"
            value={latencyStress}
            setValue={setLatencyStress}
            suffix="%"
          />
          <ScenarioSlider
            label="What if demand rises?"
            value={demandShock}
            setValue={setDemandShock}
            suffix="%"
          />
          <ScenarioSlider
            label="Certainty target"
            value={certainty}
            setValue={setCertainty}
            suffix="%"
          />
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <TrustFact
              ...
              value={stability > 65 ? "Monitor" : "Escalate"}
            />
          </div>
        </div>
            ...
            {agents.map((agent) => (
              <div key={agent.name} className="rounded-2xl border border-white bg-white/90 p-4">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="rounded-full bg-blue-100 px-2 py-1 font-mono text-[10px] text-blue-800">
                    {agent.weight}%
                  </span>
                </div>
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[1.15fr_0.85fr]">
        <div className="rounded-2xl border border-slate-200 bg-white p-4">
          ...
          <div className="mt-4 grid gap-3">
            {dag.map(([title, detail], index) => (
              <div
                key={title}
                className="grid gap-3 rounded-2xl border border-slate-100 bg-slate-50 p-3 md:grid-cols-[2rem_1fr]"
              >
                <div className="flex h-8 w-8 items-center justify-center rounded-full bg-slate-950 font-mono text-xs font-bold text-white">
                  {index + 1}
                </div>
                <div>
          ...
          {showExpertTrace && (
            <p className="mt-3 rounded-xl border border-purple-100 bg-purple-50 p-3 font-mono text-[11px] text-purple-950">
              Expert trace: coherence={coherence.toFixed(4)} · φ-resonance={resonance.toFixed(4)} ·
              counterfactual_stability={stability.toFixed(2)}
            </p>
          )}
        </div>
        ...
        <div className="rounded-2xl border border-emerald-100 bg-emerald-50 p-4">
          ...
          <p className="mt-2 text-sm leading-6 text-emerald-900">
            High-resonance quantum-causal simulation is deliberately more expensive than a standard
            computation because it keeps the claim evidence-bound and auditable.
          </p>
          <div className="mt-4 grid gap-3">
            <TrustFact
              label="Standard computation"
              value={`${standardCost.toFixed(2)} capacity units`}
            />
            <TrustFact
              label="Quantum-causal run"
              value={`${causalCost.toFixed(2)} capacity units`}
            />
            <TrustFact
              label="Certainty premium"
              value={`${((causalCost / standardCost - 1) * 100).toFixed(0)}%`}
            />
          </div>
        </div>
      </div>
