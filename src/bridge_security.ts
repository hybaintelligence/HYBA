...
export function validateProductionJwtSecret(secret: string | undefined | null): { ok: true } | { ok: false; reason: string } {
  ...
  if (byteLength < MIN_PRODUCTION_JWT_SECRET_BYTES) {
    return { ok: false, reason: `JWT_SECRET must be at least ${MIN_PRODUCTION_JWT_SECRET_BYTES} bytes (got ${byteLength})` };
  }
  return { ok: true };
}
