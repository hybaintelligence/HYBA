...
const CEOTerminal: React.FC<CEOTerminalProps> = ({ token, autoScroll = true, maxEvents = 100 }) => {
  ...
  // PHASE 5: Fetch agent status
  useEffect(() => {
    const fetchAgentStatus = async () => {
      try {
        const response = await fetch("/api/security/regeneration/agents/status", {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
        if (response.ok) {
          const data = await response.json();
          setAgentStatus(data.orchestrator_status);
        }
      } catch (e) {
  ...
  return (
    <div className="bg-slate-900 rounded-xl border border-slate-700 overflow-hidden">
      ...
      <div className="bg-slate-800 border-b border-slate-700 p-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* PHASE 5: Agent Hierarchy Toggle */}
            <button
      ...
      {/* PHASE 5: Agent Hierarchy Panel */}
      {showAgentHierarchy && agentStatus && (
        <div className="bg-slate-800/50 border-b border-slate-700 p-4">
          ...
          <div className="mt-3 flex items-center gap-4 text-xs text-slate-400">
            ...
            <span>Escalation Queue: {agentStatus.escalation_queue_size}</span>
          </div>
        </div>
      )}
