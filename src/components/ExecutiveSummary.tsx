...
export function ExecutiveSummary({
  ...
  return (
    <div className="executive-summary-card rounded-xl p-6">
      <div className="mb-4">
        <h3 className="text-sm font-black uppercase tracking-[0.18em] text-slate-900 executive-typography">
          Executive Summary
        </h3>
        <p className="mt-1 text-xs text-slate-500">
          Real-time operational overview and key performance indicators
        </p>
      </div>
        ...
        {metrics.map((metric, idx) => (
          <div key={idx} className="kpi-card rounded-lg p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <div
                className="rounded-full bg-slate-50 p-1.5"
                style={{ color: metric.positive ? "#003666" : "#DC2626" }}
              >
                {metric.icon}
              </div>
            </div>
            ...
            <div
              ...
              style={{ color: metric.positive ? "#16A34A" : "#DC2626" }}
            >
              {metric.trend}
            </div>
          </div>
