...
export const ConsoleMetrics: React.FC<ConsoleMetricsProps> = ({ state, onRefresh, isSyncing }) => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      ...
      <div
        id="metric-speedup"
        className="bg-white border-l-4 border-clicquot-orange border-y border-r border-[#E2E4E9] rounded-xl p-5 flex flex-col justify-between transition-colors duration-200 shadow-sm"
      >
        <div className="flex items-center justify-between">
          <span className="text-xs font-mono text-lux-slate tracking-wider font-semibold uppercase">
            Search Efficiency
          </span>
        ...
        <div className="my-3">
          <div className="flex items-baseline gap-2">
            <div className="text-2xl font-bold font-mono text-oxford tracking-tight">
              {state.actualSpeedupFactor.toFixed(1)}
              <span className="text-clicquot-orange text-sm ml-1 font-sans">x</span>
            </div>
      ...
      <div
        id="metric-hashrate"
        className="bg-white border-l-4 border-oxford border-y border-r border-[#E2E4E9] rounded-xl p-5 flex flex-col justify-between transition-colors duration-200 shadow-sm relative overflow-hidden"
      >
        <div className="flex items-center justify-between z-10 relative">
          <span className="text-xs font-mono text-lux-slate tracking-wider font-semibold uppercase">
            Hashrate Equivalence
          </span>
          <Radio className="text-oxford w-4.5 h-4.5" />
        </div>
        <div className="my-3 flex-1 flex flex-col justify-center z-10 relative">
          <div className="text-2xl font-bold font-mono text-oxford tracking-tight">
            {state.currentHashrate.toFixed(1)}{" "}
            <span className="text-lux-slate text-xs font-sans">EHS</span>
          </div>
          <p className="text-xs text-lux-slate mt-1">
            Estimated equivalence metric from configured runtime telemetry
          </p>
        </div>
      ...
      <div
        id="metric-power"
        className="bg-white border-l-4 border-clicquot-gold border-y border-r border-[#E2E4E9] rounded-xl p-5 flex flex-col justify-between transition-colors duration-200 shadow-sm"
      >
        <div className="flex items-center justify-between">
          <span className="text-xs font-mono text-lux-slate tracking-wider font-semibold uppercase">
            Thermal Footprint
        ...
        <div className="my-3">
          ...
          <p className="text-xs text-lux-slate mt-1">
            Electrical load constraint for the current runtime configuration.
          </p>
        </div>
        <div className="text-[10px] font-mono text-lux-slate/80 flex justify-between border-t border-sand-dark pt-2 mt-1">
      ...
      <div
        id="metric-pool"
        className="bg-white border-l-4 border-mckinsey-light border-y border-r border-[#E2E4E9] rounded-xl p-5 flex flex-col justify-between transition-colors duration-200 shadow-sm"
      >
        <div className="flex items-center justify-between">
          <span className="text-xs font-mono text-lux-slate tracking-wider font-semibold uppercase">
            Active Stratum Link
        ...
        <div className="my-3">
          ...
          <p className="text-xs text-lux-slate mt-1">
            Configured target-routing surface for the active Stratum endpoint
          </p>
        </div>
        <div className="text-[10px] font-mono text-lux-slate/80 flex justify-between border-t border-sand-dark pt-2 mt-2">
          ...
          >
            <RefreshCcw
              className={`w-3 h-3 ${isSyncing ? "animate-spin text-clicquot-orange" : ""}`}
            />
