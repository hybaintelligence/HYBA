...
export function MetricExplainerCard({
  ...
  return (
    <div className="rounded-xl border border-blue-100 bg-blue-50/70 p-4 text-sm text-slate-700">
      {value && (
        <p className="mb-1 font-mono text-[11px] font-bold uppercase tracking-[0.14em] text-blue-800">
          {value}
        </p>
      )}
      ...
      <p className="mt-1">
        ...
        {translation.operationalImplication}
      </p>
      <p className="mt-1">
...
export function ClaimBoundaryBadge({
  boundary = "advisory intelligence; human approval required",
}: {
  boundary?: string;
}) {
  return (
    <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-950">
      ...
      <p>Claim boundary: {boundary}. Human approval is required before material action.</p>
    </div>
  );
}

export function EvidenceBoundAnswer({
  seal,
  evidenceSeal,
  claimBoundary = "advisory intelligence",
  ...
  return (
    <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-950">
      ...
      <p className="mt-1">
        Seal: {seal || evidenceSeal || "required"} · Boundary: {claimBoundary} · Invariants:{" "}
        {invariantStatus} · Source: {source || dataSource || "live telemetry only"}
      </p>
    </div>
  );
}
...
export function ProofExplainer({
  ...
  return (
    <EvidenceBoundAnswer
      seal={seal}
      invariantStatus={invariantSummary}
      source={source}
      claimBoundary="proof-backed advisory intelligence"
    />
  );
}
