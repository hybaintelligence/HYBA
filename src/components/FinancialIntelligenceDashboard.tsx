  ...
  const [layerStatus, setLayerStatus] = useState<LayerStatus[]>([
    {
      name: 'Core Layer',
      status: 'operational',
      modules: ['Regime Shift', 'Liquidity Topology', 'Causal Inference', 'Volatility Geometry', 'Alpha Mining', 'Risk Surface'],
      lastUpdate: new Date().toISOString()
    },
    {
      name: 'Autonomic Layer',
      status: 'operational',
      modules: ['Drift Detection', 'Manifold Integrity', 'Curvature Anomaly', 'Factor Repair', 'Entropy Optimization', 'Topology Rewiring'],
      lastUpdate: new Date().toISOString()
    },
    {
            ...
            {layerStatus.map((layer) => (
              <Card key={layer.name}>
                <CardHeader>
                  ...
                  <CardDescription>
                    {layer.modules.length} modules operational
                  </CardDescription>
                </CardHeader>
                <CardContent>
        ...
        <TabsContent value="core" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Market regime detection, liquidity topology, causal inference, volatility geometry, alpha mining, and risk surface reconstruction
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {layerStatus[0].modules.map((module) => (
                  <Card key={module}>
                    ...
                    <CardContent>
                      <p className="text-xs text-muted-foreground">
                        {module === 'Regime Shift' && 'Detect market regime changes using φ-density windows and persistent homology'}
                        {module === 'Liquidity Topology' && 'Map liquidity as a dynamic topological surface with curvature analysis'}
                        {module === 'Causal Inference' && 'Discover and validate causal relationships using PC algorithm'}
                        {module === 'Volatility Geometry' && 'Model volatility as a geometric object using rough path theory'}
                        {module === 'Alpha Mining' && 'Discover predictive signals using information theory'}
                        {module === 'Risk Surface' && 'Reconstruct risk as a high-dimensional surface'}
                      </p>
        ...
        <TabsContent value="autonomic" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Drift detection, manifold integrity checks, curvature-based anomaly detection, factor model self-repair, entropy optimization, and topology-aware rewiring
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {layerStatus[1].modules.map((module) => (
                  <Card key={module}>
                    ...
                    <CardContent>
                      <p className="text-xs text-muted-foreground">
                        {module === 'Drift Detection' && 'Detect when models drift from intended behavior'}
                        {module === 'Manifold Integrity' && 'Verify latent manifolds maintain topological integrity'}
                        {module === 'Curvature Anomaly' && 'Detect anomalies through curvature analysis'}
                        {module === 'Factor Repair' && 'Automatically repair degraded factor models'}
                        {module === 'Entropy Optimization' && 'Optimize parameters to minimize entropy production'}
                        {module === 'Topology Rewiring' && 'Rewire system based on topological changes'}
                      </p>
                      <Button className="mt-3 w-full" variant="outline" size="sm">
                        Execute
                      </Button>
        ...
        <TabsContent value="sovereign" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Kernel-verified reasoning, stress-test harness, and systemic-risk topology maps with cryptographic evidence sealing
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {layerStatus[2].modules.map((module) => (
                  <Card key={module}>
                    ...
                    <CardContent>
                      <p className="text-xs text-muted-foreground">
                        {module === 'Kernel Reasoning' && 'Verify reasoning chains using kernel methods'}
                        {module === 'Stress Test Harness' && 'Comprehensive stress testing with Monte Carlo simulation'}
                        {module === 'Systemic Risk Mapping' && 'Map systemic risk as a topological network'}
                      </p>
                      <Button className="mt-3 w-full" variant="outline" size="sm">
                        Verify
                      </Button>
