...
export default function HistoricalDataSection({ telemetry }: { telemetry?: TelemetryData | null }) {
  ...
  const calculateTrend = (data: HistoricalDataPoint[], metric: string) => {
    ...
    const recentAvg = recent.reduce((sum, d) => sum + (metric === "hashrate" ? d.hashrate : metric === "shares" ? d.shares : metric === "difficulty" ? d.difficulty : d.powerConsumption), 0) / recent.length;
    const olderAvg = older.reduce((sum, d) => sum + (metric === "hashrate" ? d.hashrate : metric === "shares" ? d.shares : metric === "difficulty" ? d.difficulty : d.powerConsumption), 0) / older.length;
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className={`flex items-center gap-1 text-sm ${trend.trend === "up" ? "text-emerald-600" : trend.trend === "down" ? "text-red-600" : "text-slate-600"}`}>
              {trend.trend === "up" ? <TrendingUp className="h-4 w-4" /> : trend.trend === "down" ? <TrendingDown className="h-4 w-4" /> : null}
              <span>{trend.change.toFixed(1)}%</span>
            </div>
          </div>
          <p className="text-xs text-slate-600 uppercase tracking-wider">Current Hashrate</p>
          <p className="text-2xl font-black text-slate-900">{latestData?.hashrate.toFixed(2)} EH/s</p>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <p className="text-2xl font-black text-slate-900">{latestData?.shares.toLocaleString()}</p>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <p className="text-2xl font-black text-slate-900">{(latestData?.difficulty! / 1e12).toFixed(2)}T</p>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <p className="text-2xl font-black text-slate-900">{latestData?.powerConsumption.toFixed(0)}W</p>
        </div>
      </div>
        ...
        <div className="space-y-3">
          <div className="flex items-start gap-3 p-3 bg-emerald-50 rounded-lg border border-emerald-200">
            ...
            <div>
              ...
              <p className="text-sm text-slate-600">Successfully mined block #845210 with 12.5 BTC reward</p>
          ...
          <div className="flex items-start gap-3 p-3 bg-blue-50 rounded-lg border border-blue-200">
            ...
            <div>
              ...
              <p className="text-sm text-slate-600">Switched to Foundry USA pool for better efficiency</p>
          ...
          <div className="flex items-start gap-3 p-3 bg-amber-50 rounded-lg border border-amber-200">
            ...
            <div>
              ...
              <p className="text-sm text-slate-600">Power scale adjusted to 1.2x for optimal performance</p>
