...
const qiaPresets: Record<QiaPresetName, QiaPreset> = {
  "Starter QI Rail": {
    ...
    governance: "balanced",
    description:
      "Development-friendly QI rail for exploration and testing with balanced governance.",
  },
  "Enterprise Strategy Rail": {
    tier: "production",
    isolation: "single-tenant",
  ...
  "Regulated Evidence Rail": {
    ...
    governance: "strict",
    description:
      "High-compliance QI rail for regulated industries with strict governance and evidence requirements.",
  },
  "Sovereign Oracle Rail": {
    tier: "sovereign",
    ...
    governance: "strict",
    description:
      "Maximum isolation and sovereignty with strict governance for critical strategic intelligence.",
  },
  "Research/Expert QI Rail": {
    tier: "developer",
    isolation: "single-tenant",
    ...
    governance: "relaxed",
    description:
      "Expert research rail with relaxed governance for quantum experimentation and advanced algorithms.",
  },
};
...
export default function QaaSComputerManager({ token }: QaaSComputerManagerProps) {
  ...
  const [provisionForm, setProvisionForm] = useState<ProvisionFaultTolerantComputerRequest>({
    name: "",
    ...qiaPresets["Enterprise Strategy Rail"],
  });
  ...
  const applyPreset = (presetName: QiaPresetName) => {
    ...
    setProvisionForm((current) => ({
      ...
      isolation:
        preset.isolation === "sovereign-isolated" && !isAdmin ? "single-tenant" : preset.isolation,
    }));
  };
    ...
    <div className="space-y-6">
      <div className="rounded-3xl border border-violet-200 bg-gradient-to-br from-violet-50 via-white to-indigo-50 p-6 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <p className="font-mono text-xs font-bold uppercase tracking-[0.24em] text-violet-700">
              Quantum Intelligence · deep brain
            </p>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-700">
              Calm strategic reasoning rail for active simulations, φ-resonance stability, evidence
              seals, causal stability, proof weight, and counterfactual depth. QI explainers
              intentionally avoid process-log language.
            </p>
          </div>
          <button
            onClick={() => setShowProvisionModal(true)}
            className="flex items-center gap-2 rounded-xl bg-violet-600 px-4 py-2 font-semibold text-white hover:bg-violet-700"
          >
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-4">
          {(
            [
              [
                "Active simulations",
                computers.filter((c) => c.state === "running").length,
                MoonStar,
              ],
            ...
            return (
              ...
              >
                ...
                <p className="mt-2 font-mono text-[10px] uppercase tracking-[0.16em] text-violet-700">
                  {label}
                </p>
        ...
        <div className="grid gap-4">
          {computers.map((computer) => (
            <div
              ...
              className="rounded-lg border border-violet-100 bg-white p-6 shadow-sm"
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  ...
                  <div className="mt-3 rounded-lg border border-violet-100 bg-violet-50 p-3 text-xs text-violet-950">
                    <strong>QI proof tier:</strong> Gold-Standard Seal · immutable evidence seal and
                    invariant proof explain why a strategic decision is true within its claim
                    boundary.
                  </div>
                </div>
                <div className="flex gap-2">
                  {computer.state === "stopped" || computer.state === "provisioned" ? (
                    <button
                      ...
                      className="flex items-center gap-1 rounded-lg bg-violet-600 px-3 py-2 text-white"
                    >
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 rounded-lg bg-slate-600 px-3 py-2 text-white"
                    >
      ...
      {selectedComputer && (
        <div className="rounded-2xl border border-violet-200 bg-violet-50 p-4 text-sm text-violet-950">
          <strong>Selected QI rail:</strong> {selectedComputer.name} · Active Simulations are
          provisioned independently from CI operational jobs.
        </div>
      )}
      {showProvisionModal && (
            ...
            <form onSubmit={handleProvision} className="space-y-4">
              <div className="rounded-xl border border-violet-100 bg-violet-50 p-4">
                ...
                <div className="mt-3 grid gap-2 sm:grid-cols-2">
                  {presetNames.map((name) => (
                    <button
                      ...
                      className={`rounded-lg border px-3 py-2 text-left text-xs font-semibold ${selectedPreset === name ? "border-violet-500 bg-violet-100 text-violet-950" : "border-violet-200 bg-white text-violet-900"}`}
                    >
                      {name}
                    </button>
                  ))}
                ...
                <p className="mt-2 text-xs text-violet-800">
                  {isExpertMode
                    ? "Expert lens exposes raw quantum intelligence parameters."
                    : "Business lenses use presets; raw quantum parameters are optional."}
                </p>
              </div>
              <div>
