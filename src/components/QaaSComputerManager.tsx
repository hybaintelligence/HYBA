...
export default function QaaSComputerManager({ token }: QaaSComputerManagerProps) {
  ...
  useEffect(() => { fetchComputers(); }, [token]);
  ...
  const getStateColor = (state: ComputerState) => state === "running" ? "text-violet-700 bg-violet-50 border-violet-200" : state === "provisioned" ? "text-blue-700 bg-blue-50 border-blue-200" : "text-slate-600 bg-slate-50 border-slate-200";
  const getTierColor = (tier: QaaSTier) => tier === "sovereign" ? "text-purple-700 bg-purple-50 border-purple-200" : tier === "production" ? "text-blue-700 bg-blue-50 border-blue-200" : "text-slate-600 bg-slate-50 border-slate-200";
