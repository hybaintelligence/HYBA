...
export const CoherenceScatterPlot: React.FC<CoherenceScatterPlotProps> = ({
  coherence,
  resonance,
  className,
}) => {
  ...
  return (
    <div
      className={`bg-white border border-[#E2E4E9] rounded-xl p-5 shadow-sm relative ${className || ""}`}
    >
          ...
          <g transform={`translate(${margin.left},${margin.top})`}>
            {renderedXScale && (
              <g
                ...
                ref={(el) => {
                  if (el) d3.select(el).call(d3.axisBottom(renderedXScale).ticks(5));
                }}
            ...
            {renderedYScale && (
              <g
                ref={(el) => {
                  if (el) d3.select(el).call(d3.axisLeft(renderedYScale).ticks(5));
                }}
                className="text-[10px] font-mono text-[#64748B] opacity-50"
              />
            )}
                ...
                data.map((d, i) => (
                  <React.Fragment key={d.time.getTime() + i}>
                    <motion.circle
                      ...
                      fill="#0A5C91"
                      onMouseEnter={(e) =>
                        setHoveredPoint({ point: d, type: "coherence", x: e.clientX, y: e.clientY })
                      }
                    ...
                    <motion.circle
                      ...
                      fill="#e36c0a"
                      onMouseEnter={(e) =>
                        setHoveredPoint({ point: d, type: "resonance", x: e.clientX, y: e.clientY })
                      }
                      onMouseLeave={() => setHoveredPoint(null)}
                      style={{ cursor: "pointer" }}
                    />
            ...
            <div className="flex flex-col gap-1">
              <div className="flex justify-between gap-4">
                ...
                <span className="font-bold">
                  {(hoveredPoint.type === "coherence"
                    ? hoveredPoint.point.coherence
                    : hoveredPoint.point.resonance
                  ).toFixed(4)}
                </span>
              </div>
              <div className="flex justify-between gap-4">
