...
export const NetworkToast: React.FC<NetworkToastProps> = ({ isConnected, latencyMs, isDismissed, onDismiss }) => {
        ...
        >
          ...
          <div className="flex-1">
             <h4 className="font-mono font-bold text-sm">
                {!isConnected ? "Connection Lost to HYBA Unified Backend" : "High Network Latency Detected"}
             </h4>
          ...
          <button onClick={onDismiss} className="text-red-400 hover:text-white transition-colors cursor-pointer">
            <X className="w-4 h-4" />
          </button>
        </motion.div>
      )}
