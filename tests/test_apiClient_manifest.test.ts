...
describe("frontend API command manifest", () => {
  it("extracts a machine-readable route manifest for every apiClient helper-backed command", () => {
    const outPath = join(mkdtempSync(join(tmpdir(), "hyba-api-manifest-")), "frontend_api_command_manifest.generated.json");
    execFileSync(process.execPath, ["scripts/extract_api_client_manifest.mjs", "--out", outPath], { stdio: "pipe" });
    const manifest = JSON.parse(readFileSync(outPath, "utf8")) as ManifestRow[];
