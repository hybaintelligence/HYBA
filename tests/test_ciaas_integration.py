...
class TestPackages:
    ...
    def test_government_security_package(self):
        ...
        logger.info(
            f"✓ Government security optimization: {len(result['recommended_actions'])} actions"
        )
    ...
    def test_energy_optimization_package(self):
        ...
        logger.info(
            f"✓ Energy optimization: {len(result['autonomous_actions'])} control actions"
        )
    ...
    def test_protein_folding_package(self):
        ...
        logger.info(
            f"✓ Protein folding: {result['prediction_time_minutes']} min prediction"
        )
...
class TestEndToEnd:
    ...
    def test_government_workflow(self):
        ...
        logger.info(
            f"2. Optimization complete: {len(result['recommended_actions'])} recommendations"
        )
    ...
    def test_energy_workflow(self):
        ...
        logger.info(
            f"2. Optimization complete: {result['efficiency_improvement']:.1f}% efficiency gain"
        )
    ...
    def test_drug_discovery_workflow(self):
        ...
        logger.info(
            f"3. Protein structure prediction: {protein_result['prediction_time_minutes']} min"
        )
        logger.info(
            f"   - Binding sites: {protein_result['druggability']['binding_sites_count']}"
        )
        logger.info(
            f"   - Druggability: {protein_result['druggability']['overall_score']:.2f}"
        )
    ...
    def _rank_compounds_for_target(
        self, compounds: pd.DataFrame, target_result: dict
    ) -> list:
            ...
            if "fingerprint" in cmpd:
                ...
                if isinstance(fp, np.ndarray):
                    ...
                    hydrophobic_fraction = (
                        np.mean(fp) if isinstance(fp, np.ndarray) else 0.5
                    )

                    # Combine with target druggability
                    binding_score = (
                        hydrophobic_fraction
                        * target_result["druggability"]["overall_score"]
                    )
