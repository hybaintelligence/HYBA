...
def test_state_discrimination():
    ...
    if rel_dist_high_low > discrimination_threshold:
        print(
            f"✓ High vs Low: DISCRIMINABLE (relative distance {rel_dist_high_low:.3f} > {discrimination_threshold})"
        )
    else:
        print(
            f"✗ High vs Low: NOT DISCRIMINABLE (relative distance {rel_dist_high_low:.3f} ≤ {discrimination_threshold})"
        )

    if rel_dist_high_mixed > discrimination_threshold:
        print(
            f"✓ High vs Mixed: DISCRIMINABLE (relative distance {rel_dist_high_mixed:.3f} > {discrimination_threshold})"
        )
    else:
        print(
            f"✗ High vs Mixed: NOT DISCRIMINABLE (relative distance {rel_dist_high_mixed:.3f} ≤ {discrimination_threshold})"
        )

    if rel_dist_low_mixed > discrimination_threshold:
        print(
            f"✓ Low vs Mixed: DISCRIMINABLE (relative distance {rel_dist_low_mixed:.3f} > {discrimination_threshold})"
        )
    else:
        print(
            f"✗ Low vs Mixed: NOT DISCRIMINABLE (relative distance {rel_dist_low_mixed:.3f} ≤ {discrimination_threshold})"
        )
    ...
    if MATPLOTLIB_AVAILABLE:
        try:
            ...
            plt.tight_layout()
            plt.savefig(
                "/Users/demouser/Desktop/HYBA_FULLSTACK/memory_fabric_discrimination.png",
                dpi=150,
                bbox_inches="tight",
            )
