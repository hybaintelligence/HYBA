...
class PulviniPhiMemoryCompressionTests(unittest.TestCase):
    ...
    def test_fabric_uses_phi_folded_kernel_state(self) -> None:
        ...
        for reward, path in [(1.0, [0, 20, 25, 30, 31]), (0.5, [1, 21, 26, 27, 16]), (0.8, [2, 22, 23, 8, 9])]:
        ...
        self.assertIn(snapshot["compression"]["compression_strategy"], {"phi_fold", "sparse_passthrough"})
        compression_dict = snapshot["compression"]
        self.assertFalse(np.isnan(compression_dict.get("reconstruction_error", 0.0)), "NaN in reconstruction error")
        self.assertFalse(np.isinf(compression_dict.get("reconstruction_error", 0.0)), "Inf in reconstruction error")
    ...
    def test_round_trip_high_entropy_random_vector(self) -> None:
        ...
        self.assertTrue(np.allclose(result.reconstructed.reshape(-1), payload, rtol=0, atol=engine.tolerance))
    ...
    def test_reconstruction_error_is_finite_for_all_adversarial_cases(self) -> None:
        ...
        cases = [
            ...
            np.ones(32, dtype=np.float64) * np.finfo(np.float64).max / 2,  # near-max: clamped, finite error expected
