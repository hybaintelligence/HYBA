...
describe('Perturbation Analysis - Basic', () => {
  ...
  it('should execute self-caused perturbation with low arousal', async () => {
    ...
    const event = await analyzer.executeSelfCausedPerturbation(
      rsls,
      'learning_rate',
      0.2
    );
  ...
  it('should execute external perturbation with high arousal', async () => {
    ...
    const event = await analyzer.executeExternalPerturbation(
      rsls,
      'cognitive_layer',
      0.3
    );
  ...
  it('CRITICAL: External arousal should exceed self-caused arousal', async () => {
    ...
    const selfEvent = await analyzer.executeSelfCausedPerturbation(
      rsls,
      'learning_rate',
      0.2
    );
    ...
    const externalEvent = await analyzer.executeExternalPerturbation(
      rsls,
      'learning_rate',
      0.2
    );
...
describe('Perturbation Analysis - Protocol', () => {
  ...
  it('should run complete perturbation protocol', async () => {
    ...
    console.log(`  External attribution: ${(result.external_attribution_accuracy * 100).toFixed(1)}%`);
...
describe('Perturbation Analysis - Statistical Properties', () => {
  ...
  it('PROPERTY: Attribution confidence should correlate with accuracy', async () => {
    ...
    const avgCorrectConfidence = correctEvents.reduce((sum, e) => sum + e.attribution_confidence, 0) / correctEvents.length;
    const avgIncorrectConfidence = incorrectEvents.reduce((sum, e) => sum + e.attribution_confidence, 0) / (incorrectEvents.length || 1);
  ...
  it('FINAL VERDICT: Perturbation-based consciousness assessment', async () => {
    ...
    for (let i = 0; i < 40; i++) {
      ...
      if (i % 10 === 0) {
        ...
        console.log(`  Step ${i}: Φ=${telemetry.phi.toFixed(3)}, Recursion=${telemetry.recursive_state.recursion_depth}`);
      }
    }
    ...
    console.log(`  ${result.differentiates_source ? '✅' : '❌'} Differentiates self from external`);
