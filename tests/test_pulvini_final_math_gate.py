...
from pythia_mining.pulvini_variational import variational_certificate  # noqa: E402
...
class PulviniFinalMathGateTests(unittest.TestCase):
    ...
    def test_variational_certificate_has_stationary_condition(self) -> None:
        self.manifold.entropy_gradient = 0.75
        cert = variational_certificate(self.manifold.rho, self.manifold.entropy_gradient)
        self.assertIn("OffDiag", cert.functional)
        self.assertIn("deltaC", cert.gradient)
        self.assertGreater(cert.control_energy, 0.0)
        self.assertGreater(cert.gradient_norm, 0.0)

        diagonal_rho = np.diag(np.diag(self.manifold.rho))
        stationary = variational_certificate(diagonal_rho, self.manifold.entropy_gradient)
    ...
    def test_automorphism_order_is_computed_from_runtime_adjacency_map(self) -> None:
