...
test.beforeEach(async ({ page }) => {
  ...
  await page.route("**/api/auth/login", (route) =>
    route.fulfill({
      json: { success: true, token: "mock-token", user: { username: "operator", role: "miner" } },
    }),
  );
});

test("operator can register, log in, and log out through accessible form controls", async ({
  page,
}) => {
  await page.goto("/");
