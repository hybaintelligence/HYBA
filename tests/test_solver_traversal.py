...
async def test_solver_traversal():
    ...
    if len(nonces) == 1:
        ...
        print("  → Solver may be using cached nonce instead of traversing plan")
        return False
    elif len(nonces) < 5:
        print(f"  ⚠ LOW NONCE VARIETY: Only {len(nonces)} unique nonces in 10 runs")
        print("  → Possible partial short-circuiting or limited search space")
        return False
    else:
        print(f"  ✓ NO SHORT-CIRCUIT: {len(nonces)} unique nonces in 10 runs")
        print("  → Solver is genuinely traversing the compressed plan")
    ...
    if np.std(solve_times) < 0.1:
        ...
        print("  → Consistent with genuine computation")
    ...
    if mean_time < 0.1:
        ...
        print("  → Likely short-circuiting to cached nonce")
        return False
    elif mean_time > 100:
        print(f"  ⚠ SLOW TRAVERSAL: {mean_time:.2f}ms mean")
        print("  → May be traversing full space without compression benefits")
    else:
        print(f"  ✓ REASONABLE TIMING: {mean_time:.2f}ms mean")
        print("  → Consistent with compressed plan traversal")
