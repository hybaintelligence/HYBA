...
class ManifoldStressAnalyzer:
    ...
    def compute_fisher_information_matrix(self, rho: NDArray[np.complex128]) -> NDArray[np.float64]:
        """
    ...
    def compute_geometric_stability(
        ...
        spectral_gap = rho_eigvals[-1] - rho_eigvals[-2] if len(rho_eigvals) > 1 else 0.0
...
def test_manifold_limit_discovery():
    ...
    print(f"{'Dimension':<10} {'Stability':<12} {'Ricci':<12} {'QFI-κ':<12} "
    ...
    for dim in dimensions:
        ...
        try:
            ...
            print(f"{dim:<10} {stability:>11.6f} {ricci:>11.6f} {condition_number:>11.2e} "
            ...
            if stability < 0.1:
                ...
                print(f"    QFI condition number: {condition_number:.2e} (>1e6 = ill-conditioned)")
                break
                
        except Exception as e:
            print(f"{dim:<10} {'ERROR':<12} {'ERROR':<12} {'ERROR':<12} "
