...
class QuantumAdversary:
    ...
    def inject_symmetry_breaking_noise(
        ...
        orbit_violation = self._measure_orbit_violation(original_orbits, perturbed_orbits)
    ...
    def _inject_adversarial_noise(self, entropy_level: float) -> NDArray[np.float64]:
        ...
        for node in target_nodes:
            if node < canonical_map.shape[0]:
                ...
                noise[node] = self.rng.randn(canonical_map.shape[1]) * entropy_level * 2.0
    ...
    def _measure_orbit_violation(
        self, 
        original_orbits: List[List[int]], 
        perturbed_orbits: List[List[int]]
    ) -> float:
        """
...
def test_symmetry_resilience_under_noise():
    ...
    successful_repairs = sum(1 for r in results 
                            if r['repair_record'] and r['repair_record']['success'])
    ...
    print(f"   Success Rate: {100*(successful_repairs + no_repair_needed)/total_tests:.1f}%")
    ...
    for attack_type in attack_types:
        ...
        type_successes = sum(1 for r in type_results 
                           if not r['violation_detected'] or 
                           (r['repair_record'] and r['repair_record']['success']))
