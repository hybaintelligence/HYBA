        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.FunctionDef):
                    if any(
                        ...
                        for keyword in ["update", "adjust", "tune", "optimize", "adapt", "learn"]
                    ):
            ...
            for pattern in accumulation_patterns:
                ...
                for match in matches:
                    ...
                    loops.append({"type": "accumulation", "pattern": pattern, "line": line_num})
        ...
        for child in ast.walk(node):
            if isinstance(child, ast.Name):
                ...
                if isinstance(child.value, ast.Name):
                    names.add("{child.value.id}.{child.attr}")
        return names
        ...
        if isinstance(node, ast.Assign):
            for target in node.targets:
                if isinstance(target, ast.Attribute):
                    if isinstance(target.value, ast.Name) and target.value.id == "sel":
                        return True
        elif isinstance(node, ast.AugAssign):
            if isinstance(node.target, ast.Attribute):
                if isinstance(node.target.value, ast.Name) and node.target.value.id == "sel":
                    return True
        return False
    ...
    def analyze_file(self, file_path: Path) -> Dict:
        ...
        try:
            ...
            for pattern, label in adaptive_patterns:
                ...
                for match in matches:
                    line_num = content[: match.start()].count("\n") + 1
                    optimizations.append({"type": label, "line": line_num})
            ...
            for pattern, label in memory_keywords:
                ...
                for match in matches:
                    ...
                    memory_patterns.append(
                        {"type": label, "line": line_num, "context": line_content[:100]}
                    )
...
class TestAdaptiveBehaviorDeepAnalysis(unittest.TestCase):
    ...
    def test_01_feedback_loops(self):
        ...
        print("Total feedback patterns found: {total_loops}")
        print("Files with feedback loops: {len(all_results)}")

        # Show top files
        all_results.sort(key=lambda x: x.get("count", 0), reverse=True)
        print("\nTop 5 files with most feedback patterns:")
        for i, result in enumerate(all_results[:5], 1):
            print("  {i}. {result['file']}: {result['count']} patterns")
    ...
    def test_02_parameter_optimization(self):
        ...
        print("Total optimization patterns: {total_optimizations}")
        print("Files with optimization: {len(all_results)}")

        # Show examples
        if all_results:
            print("\nExample files:")
            for result in all_results[:3]:
                print("  - {result['file']}: {result['count']} patterns")
    ...
    def test_03_memory_accumulation(self):
        ...
        print("Total memory-related patterns: {total_memory}")
        print("Files with memory: {len(all_results)}")
    ...
    def test_04_self_modification_sites(self):
        ...
        print("Total self-modification patterns: {total_modifications}")
        print("Files with self-modification: {len(all_results)}")
        print("DANGEROUS sites (eval/exec): {len(dangerous_sites)}")

        # Show dangerous sites
        if dangerous_sites:
            print("\n⚠ DANGEROUS SELF-MODIFICATION SITES:")
            for site in dangerous_sites:
                print("  File: {site['file']} (line {site['line']})")
                print("  Type: {site['type']}")
                print("  Context: {site['context']}")
                print()
    ...
    def test_05_adaptive_behavior_verdict(self):
        ...
        print("\nMeasurements:")
        print("  Feedback loops:        {feedback}")
        print("  Optimization patterns: {optimizations}")
        print("  Memory patterns:       {memory}")
        print("  Self-modification:     {self_mod} ({dangerous} dangerous)")
        ...
        genuine_adaptive = (
            optimizations > 5  # Real optimization code
            and feedback > 10  # Multiple feedback loops
            and memory > 20  # Significant memory usage
        )
        ...
        if has_dangerous_self_mod:
            ...
            print("  {dangerous} eval/exec sites allow runtime code generation.")
        ...
        if genuine_adaptive and has_learning:
            ...
            conclusion = (
                "DETERMINISTIC SYSTEM: Adaptive keywords are naming conventions, not behavior."
            )
