...
async def test_quantum_integration():
    ...
    assert hasattr(
        genesis.quantum_solver, "current_config"
    ...
    cert = bures_certificate(
        genesis.overlay.manifold.rho, genesis.overlay.manifold.entropy_gradient
    )
    assert cert.metric == "Bures", "Bures certificate metric incorrect"
    assert cert.closed, "Bures gate not closed"
    print(
        f"✓ Bures certificate computed (norm: {cert.bures_norm:.4f}, stationary: {cert.stationary})"
    )
    ...
    print(
        f"✓ Manifold evolution stable (entropy change: {abs(evolved_entropy - initial_entropy):.6f})"
    )
...
async def test_mining_job_workflow():
    ...
    print(
        f"✓ Nonce plan generated (dimension: {plan.working_set_dimension}, compression: {plan.working_set_compression_ratio:.2f}x)"
    )
