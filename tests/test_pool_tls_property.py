...
from pythia_mining.pool_profiles import PoolProfileError, build_profile, validate_pool_url
...
def test_tls_required_accepts_all_tls_stratum_urls(scheme: str, port: int) -> None:
    assert validate_pool_url(f"{scheme}://pool.example:{port}", tls_required=True) == f"{scheme}://pool.example:{port}"
