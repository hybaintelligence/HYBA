...
class QuantumWorkloadSimulator:
    ...
    def execute_workload(
        self, execution_id: str, work_units: int, should_succeed: bool = True
    ) -> Dict[str, Any]:
...
class TestFullSystemIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.customer_simulator = QuantumWorkloadSimulator(
            "integration-test-customer", self.billing_manager
        )
    ...
    def test_complete_execution_lifecycle_success(self):
        ...
        result = self.customer_simulator.execute_workload(
            execution_id="exec-success-001", work_units=100, should_succeed=True
        )
    ...
    def test_complete_execution_lifecycle_failure_with_refund(self):
        ...
        result = self.customer_simulator.execute_workload(
            execution_id="exec-failure-001", work_units=200, should_succeed=False
        )
    ...
    def test_mixed_success_and_failure_workflow(self):
        ...
        exec1 = self.customer_simulator.execute_workload(
            execution_id="exec-001", work_units=100, should_succeed=True
        )
        ...
        exec2 = self.customer_simulator.execute_workload(
            execution_id="exec-002", work_units=300, should_succeed=False
        )
        self.assertEqual(exec2["status"], "failed")
        self.assertEqual(
            self.customer_simulator.quota, 900
        )  # 900 + 300 refund - 100 from exec1

        # Execute 3: Success (150 units consumed)
        exec3 = self.customer_simulator.execute_workload(
            execution_id="exec-003", work_units=150, should_succeed=True
        )
    ...
    def test_quota_exhaustion_prevents_execution(self):
        ...
        self.customer_simulator.execute_workload(
            execution_id="exec-exhaust-001", work_units=900, should_succeed=True
        )
        ...
        result = self.customer_simulator.execute_workload(
            execution_id="exec-exhaust-002", work_units=200, should_succeed=True
        )
    ...
    def test_idempotency_with_retry(self):
        ...
        result1 = self.customer_simulator.execute_workload(
            execution_id="exec-retry-001", work_units=150, should_succeed=False
        )
    ...
    def test_audit_trail_completeness(self):
        ...
        self.customer_simulator.execute_workload(
            execution_id="exec-audit-001", work_units=100, should_succeed=True
        )
        self.customer_simulator.execute_workload(
            execution_id="exec-audit-002", work_units=200, should_succeed=False
        )
        self.customer_simulator.execute_workload(
            execution_id="exec-audit-003", work_units=150, should_succeed=True
        )
    ...
    def test_evidence_seal_with_execution_record(self):
        ...
        result = self.customer_simulator.execute_workload(
            execution_id=execution_id, work_units=work_units, should_succeed=True
        )

        # Create evidence seal with execution data
        evidence_data = (
            f"{execution_id}:{self.customer_simulator.customer_id}:{work_units}"
        )
        ...
        tampered_data = (
            f"{execution_id}:{self.customer_simulator.customer_id}:{work_units + 100}"
        )
...
class TestEndToEndWithMultipleOperations(unittest.TestCase):
    ...
    def test_production_day_simulation(self):
        ...
        # Create multiple customers
        customers = [
            QuantumWorkloadSimulator(f"customer-{i}", billing_manager) for i in range(3)
        ]
        ...
        self.assertEqual(
            customers[2].quota, 700
        )  # 1000 - 300 - 100 (refunded - restores)
