...
def test_adversarial_energy_conservation_violation():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert not is_valid, "Proposal violating energy conservation should be rejected"
...
def test_adversarial_metrics_cache_poisoning():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            assert metrics1 != metrics2, "Cache should invalidate on state change"
        except Exception as e:
        ...
        for i in range(100):
            try:
                result = ctrl.reset_circuit_breaker(
                    operator_id=f"attacker_{i}", operator_reason="dos_attempt"
                )
