...
async def _benchmark_phi_solver(target: int, max_time_ms: float = 5000.0) -> tuple[Optional[int], float]:
    """
...
def _benchmark_random_search(target: int, max_time_ms: float = 5000.0) -> tuple[Optional[int], float]:
    """
...
async def test_hendrix_phi_vs_random_medium_target():
    ...
    print(f"  Random:    {random_time_ms:.2f}ms → nonce 0x{random_nonce:08x if random_nonce else 'TIMEOUT'}")
    print(f"  Speedup:   {speedup:.2f}x")
...
def test_hendrix_phi_deterministic_within_target():
    ...
    assert results[0] == results[1] == results[2], "HENDRIX proposals not deterministic under same seed"
...
def test_hendrix_phi_top_percentile_placement():
    ...
    assert percentage_in_top_5 >= 15.0, (
        f"HENDRIX-Φ guidance weak: only {percentage_in_top_5:.1f}% in top 5% "
        f"(random would be ~5%)"
    )
