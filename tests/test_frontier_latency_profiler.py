        ...
        for bottleneck_frac in [0.1, 0.5, 0.9]:
            for speedup_factor in [2, 5, 10, 50]:
                speedup = profiler.compute_amdahl_speedup(
                    bottleneck_frac, speedup_factor
                )
...
def test_amdahl_law_monotonicity(bottleneck_fraction, speedup_factor):
    ...
    if bottleneck_fraction < 1.0:
        ...
        higher_speedup = profiler.compute_amdahl_speedup(
            higher_fraction, speedup_factor
        )

        assert (
            higher_speedup >= speedup - 1e-10
...
def test_amdahl_law_speedup_scaling(fraction, base_speedup):
    ...
    assert (
        speedup_2x > speedup_1x
    ), f"Speedup should increase: {speedup_1x} -> {speedup_2x}"


@given(
    latencies=st.lists(
        st.floats(min_value=0.1, max_value=1000.0), min_size=10, max_size=100
    )
)
