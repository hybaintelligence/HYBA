...
class SuperiorityAnalyzer:
    ...
    def calculate_speed_metrics(
        self, hyba_times: List[float], classical_times: List[float]
    ) -> Dict[str, float]:
        ...
        latency_reduction = (
            (classical_mean - hyba_mean) / classical_mean if classical_mean > 0 else 0
        )
    ...
    def calculate_accuracy_metrics(
        ...
        classical_recalls: Optional[List[float]] = None,
    ) -> Dict[str, float]:
        ...
        if hyba_precisions and classical_precisions:
            precision_improvement = np.mean(hyba_precisions) - np.mean(
        ...
        if precision_improvement != 0 and recall_improvement != 0:
            hyba_f1 = (
                2
                * (np.mean(hyba_precisions) * np.mean(hyba_recalls))
                / (np.mean(hyba_precisions) + np.mean(hyba_recalls))
            )
            classical_f1 = (
                2
                * (np.mean(classical_precisions) * np.mean(classical_recalls))
                / (np.mean(classical_precisions) + np.mean(classical_recalls))
            )
    ...
    def calculate_scalability_metrics(
        self,
        hyba_times_by_size: Dict[int, float],
        classical_times_by_size: Dict[int, float],
    ) -> Dict[str, Any]:
        ...
        for i in range(1, len(sizes)):
            ...
            hyba_time_ratio = (
                hyba_times_by_size[sizes[i]] / hyba_times_by_size[sizes[i - 1]]
            )
            classical_time_ratio = (
                classical_times_by_size[sizes[i]]
                / classical_times_by_size[sizes[i - 1]]
            )
        ...
        scaling_factor = (
            np.mean(hyba_scaling) / np.mean(classical_scaling)
            if classical_scaling
            else 0
        )
        ...
        if len(sizes) >= 2:
            data_size_scaling = (
                hyba_times_by_size[sizes[-1]] / hyba_times_by_size[sizes[0]]
    ...
    def calculate_efficiency_metrics(
        ...
        classical_energy: Optional[List[float]] = None,
    ) -> Dict[str, float]:
        ...
        if hyba_energy and classical_energy:
            ...
            energy_efficiency = (
                classical_energy_mean / hyba_energy_mean if hyba_energy_mean > 0 else 0
            )

        # Cost efficiency (assuming cost scales with memory and energy)
        cost_efficiency = (
            (1.0 - memory_reduction) * (1.0 / energy_efficiency)
    ...
    def calculate_robustness_metrics(
        self,
        hyba_success_rate: float,
        classical_success_rate: float,
        hyba_recovery_time: Optional[float] = None,
        classical_recovery_time: Optional[float] = None,
    ) -> Dict[str, float]:
        ...
        if hyba_recovery_time and classical_recovery_time:
            error_recovery = (
                (classical_recovery_time - hyba_recovery_time) / classical_recovery_time
                if classical_recovery_time > 0
                else 0
            )
        ...
        resilience = (
            hyba_success_rate
            + (
                1.0 - (hyba_recovery_time / classical_recovery_time)
                if hyba_recovery_time and classical_recovery_time > 0
                else 0
            )
        ) / 2
    ...
    def statistical_validation(
        self, hyba_results: List[float], classical_results: List[float]
        ...
        std_err = np.sqrt(
            np.var(hyba_results) / len(hyba_results)
            + np.var(classical_results) / len(classical_results)
        )
        ...
        if pooled_std > 0:
            ...
            power = stats.norm.cdf(
                abs(effect_size) * np.sqrt(n1 * n2 / (n1 + n2)) - 1.96
            )
        else:
            power = 0.0
    ...
    def calculate_overall_superiority(
    ...
    def analyze_superiority(
        ...
        classical_success_rate: Optional[float] = None,
    ) -> SuperiorityMetrics:
        ...
        # Statistical validation
        p_value, ci, effect_size, significant, power = self.statistical_validation(
            hyba_accuracies, classical_accuracies
        )
...
def main():
    ...
    print(
        f"  Statistical Significance: {metrics.statistical_significance} (p={metrics.p_value:.4f})"
    )
