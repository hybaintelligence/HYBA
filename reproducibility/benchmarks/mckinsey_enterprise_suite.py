...
class SLATracker:
    ...
    def generate_sla_report(self) -> Dict:
        ...
        critical_breaches = sum(1 for b in self.breaches if b['criticality'] == 'critical')
        ...
        return {
            ...
            'average_breach_severity': np.mean([b['breach_severity'] for b in self.breaches]) if self.breaches else 0,
...
class CostAttributor:
    ...
    def generate_chargeback_report(self, department: str) -> Dict:
        ...
        return {
            ...
            'average_cost_per_service': np.mean([c['total_cost'] for c in dept_costs]) if dept_costs else 0,
            'cost_breakdown': {
...
class UnitEconomicsCalculator:
    ...
    def calculate_cac(self, sales_marketing_spend: float, customers_acquired: int) -> float:
        """Customer acquisition cost"""
        cac = sales_marketing_spend / customers_acquired if customers_acquired > 0 else 0
    ...
    def calculate_magic_number(self, arr_current: float, arr_previous: float, sm_spend: float) -> float:
    ...
    def generate_unit_economics_report(self) -> Dict:
        ...
        return {
            ...
            'ltv_cac_ratio': self.metrics['ltv'] / self.metrics['cac'] if self.metrics['cac'] > 0 else 0,
    ...
    def _assess_health(self) -> str:
        ...
        ratio = self.metrics['ltv'] / self.metrics['cac'] if self.metrics['cac'] > 0 else 0
...
class EnterpriseRiskRegistry:
    ...
    def register_risk(self, category: str, description: str, probability: float,
                     impact: float, owner: str, mitigation_plan: Optional[str] = None) -> Dict:
    ...
    def generate_risk_report(self) -> Dict:
        ...
        return {
            ...
            'top_risks': sorted(self.risks, key=lambda x: x['risk_score'], reverse=True)[:5],
        }
...
class CohortAnalytics:
    ...
    def create_cohort(self, cohort_id: str, start_date: datetime, customer_count: int,
                     attributes: Dict):
    ...
    def track_cohort_metrics(self, cohort_id: str, month_offset: int, metrics: Dict):
        ...
        if cohort_id in self.cohorts:
            ...
            current_customers = int(self.cohorts[cohort_id]['initial_size'] * retention_curve)
...
class FinancialHealthAnalyzer:
    ...
    def project_path_to_profitability(self, revenue_growth_rate: float, 
                                     expense_growth_rate: float, months: int) -> List[Dict]:
    ...
    def generate_financial_health_report(self) -> Dict:
        ...
        return {
            ...
            'gross_margin': (self.monthly_revenue - self.monthly_expenses) / self.monthly_revenue if self.monthly_revenue > 0 else 0,
            'runway_months': self.calculate_runway_months(1_000_000),  # Assuming 1M cash
        }
...
class ExecutiveDashboard:
    ...
    def add_kpi(self, category: str, name: str, value: float, unit: str, 
               target: float = None, trend: str = None):
...
def main():
    ...
    financial_health = FinancialHealthAnalyzer(500_000, 400_000)  # £500K rev, £400K expenses
    ...
    # Calculate costs
    cost_attributor.calculate_service_cost('service-1', {
        'vcpus': 4,
        'memory_gb': 16,
        'storage_gb': 100,
        'network_gb': 50,
    ...
    print("SLA Report:", json.dumps(sla_tracker.generate_sla_report(), indent=2, default=str))
    print("\nCost Attribution:", json.dumps(cost_attributor.generate_chargeback_report('Finance'), indent=2, default=str))
    print("\nUnit Economics:", json.dumps(unit_econ.generate_unit_economics_report(), indent=2, default=str))
    print("\nRisk Report:", json.dumps(risk_registry.generate_risk_report(), indent=2, default=str))
    print("\nFinancial Health:", json.dumps(financial_health.generate_financial_health_report(), indent=2, default=str))
