...
class BenchmarkComparison:
    ...
    statistical_significance: Dict[str, Tuple[float, float]] = field(default_factory=dict)
    conclusion: str = ""
        ...
        for metric, value in run.metrics.items():
            if isinstance(value, list) and len(value) > 1:
                ...
                if cv < 0.5:  # Less than 50% variation
                    ...
                    details['statistical_checks'][check_name] = f'warning (high CV={cv:.2f})'
...
class BenchmarkOrchestrator:
    ...
    def execute_benchmark(
        ...
        run = self.schedule_benchmark(benchmark_func, benchmark_name, domain, parameters)
        ...
        try:
            ...
            if validated:
                ...
                logger.warning(f"⚠️  Benchmark completed but validation failed: {benchmark_name}")
    ...
    def compare_runs(self, run1: BenchmarkRun, run2: BenchmarkRun) -> BenchmarkComparison:
        ...
        if comparison.percent_changes:
            ...
            if avg_improvement > 10:
                comparison.conclusion = f"Run 2 is {avg_improvement:.1f}% better than Run 1"
            elif avg_improvement < -10:
                comparison.conclusion = f"Run 1 is {-avg_improvement:.1f}% better than Run 2"
            else:
    ...
    def export_results(self, format: str = 'json') -> str:
        ...
        if format == 'json':
            ...
            output_file = self.output_dir / f'benchmarks_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
    ...
    def save_summary(self) -> None:
        ...
        summary = {
            ...
            'validation_rate': sum(1 for r in self.runs if r.validation_status) / len(self.runs) if self.runs else 0,
            'by_domain': {},
        }
        ...
        for domain, runs in by_domain.items():
            summary['by_domain'][domain] = {
                ...
                'avg_duration_s': statistics.mean(r.duration_seconds for r in runs) if runs else 0,
            }
