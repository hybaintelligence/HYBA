...
Status: Capability registry for controlled science programme

## Bottom line

This registry converts the controlled adaptive-systems investigation into capability IDs that can be tested, documented, preserved, and reviewed.

The purpose is to prevent narrative drift. Each capability must have:

- a capability ID;
- evidence files;
- a test command;
- a current supported claim;
- a known gap;
- a claim boundary.

No capability should advance from observation to institutional claim unless the evidence, tests, artifact, and boundary are present.
