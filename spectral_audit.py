...
def spectral_audit(matrix, label):
    ...
    if matrix.ndim == 1:
        print(f"\n{label}")
        print(f"  (1D compressed representation - skipping eigenvalue analysis)")
    ...
    if len(unique_eigvals) < len(eigvals):
        print(f"  degenerate eigenspaces: {len(eigvals) - len(unique_eigvals)} eigenvalues collapsed into {len(unique_eigvals)} unique values")
        multiplicities = []
...
def main():
    ...
    if hasattr(compression_result, 'folded') and compression_result.folded is not None:
        spectral_audit(compression_result.folded, "3. Folded Density Matrix (compressed representation)")
    ...
    print(f"  Reconstruction error from compression result: {compression_result.reconstruction_error or 'N/A'}")
