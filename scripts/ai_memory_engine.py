...
class AIMemoryEngine:
    ...
    def get_all_memories(self, memory_type: Optional[str] = None, min_confidence: float = 0.0) -> List[Dict]:
    ...
    def reason_about_mining(self, query_str: str) -> Dict[str, Any]:
        ...
        reasoning = {
            ...
            "supporting_evidence": {
                ...
                "avg_phi_resonance": sum(e["phi_resonance"] for e in evidence_sample) / len(evidence_sample) if evidence_sample else 0
            },
            "conclusion": self._form_conclusion(hypothesis, baseline, evidence_sample, query_str),
            "confidence": self._calculate_reasoning_confidence(hypothesis, baseline, evidence_sample)
        }
    ...
    def _form_conclusion(self, hypothesis: Optional[Dict], baseline: Optional[Dict], 
                        evidence: List[Dict], query: str) -> str:
        ...
        if "phi" in query.lower():
            if baseline and baseline["z_score"] > 5:
                return f"Phi^15 resonance is statistically significant (z={baseline['z_score']:.2f}). " \
        ...
        if "mining" in query.lower():
            if hypothesis:
                ...
                return f"Mining behavior analysis: {value.get('hypothesis', 'N/A')}. " \
    ...
    def _calculate_reasoning_confidence(self, hypothesis: Optional[Dict], baseline: Optional[Dict],
    ...
    def print_memory_report(self):
        ...
        for ev in evidence[:5]:
            print(f"  • Block {ev['block_height']}: Nonce {ev['nonce']}, "
