...
def demonstrate_blockchain_structure():
    ...
    print("   → Anchors search to current blockchain tip")
    print("   → 32 bytes, determines which block we're building on")
    print()
    ...
    print("   → Constructs merkle root with coinbase transaction")
    print("   → Each branch is 32 bytes, builds the transaction tree")
    print()

    print("3. BLOCK VERSION (version)")
    print(f"   {sample_job.version}")
    print("   → Consensus rules, determines valid block structure")
    print("   → 4 bytes")
    print()

    print("4. COMPACT DIFFICULTY (nbits)")
    print(f"   {sample_job.nbits}")
    print("   → Mining difficulty target")
    print("   → 4 bytes, compact representation of target")
    ...
    print("   → Time window for valid blocks")
    print("   → 4 seconds, Unix timestamp")
    print()

    print("6. NONCE (the search variable)")
    print("   → 32-bit integer (0 to 2^32-1)")
    print("   → This is what HYBA searches over")
    print("   → 4 bytes, the only variable in the block header")
    print()
...
def demonstrate_search_process(job: MiningJob):
    ...
    print("→ These are combined with coinbase parts to create the coinbase transaction")
    print()
    ...
    for i, branch in enumerate(job.merkle_branch):
        print(f"  Branch {i}: {branch[:16]}...")
    print("→ Merkle root = hash(coinbase_hash + branch_1 + branch_2 + ...)")
    print("→ This commits to all transactions in the block")
    print()
    ...
    print("  [32 bytes] Merkle root: (computed from coinbase + merkle branch)")
    print(f"  [4 bytes] Timestamp: {job.ntime}")
    print(f"  [4 bytes] Difficulty: {job.nbits}")
    print("  [4 bytes] NONCE: (search variable)")
    print("  Total: 80 bytes")
    print()
