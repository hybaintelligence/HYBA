...
def collect_logs(service: str) -> str:
    ...
    try:
        result = run_command(
            ["docker-compose", "-f", "docker-compose.production.yml", "--env-file", ".env.docker", "logs", "--tail=100", service],
            check=False
        )
...
def main() -> int:
    ...
    result = run_command(
        ["docker-compose", "-f", "docker-compose.production.yml", "--env-file", ".env.docker", "up", "-d", "--build"],
        check=False
    )
    ...
    for i in range(snapshots_to_collect):
        ...
        print(f"[{snapshot_time}] Snapshot {i + 1}/{snapshots_to_collect} " +
        ...
        snapshot["health_checks"]["substrate"] = collect_health_check(
            "http://localhost:3001/api/substrate",
            "substrate"
        )
        ...
        ps_result = run_command(
            ["docker-compose", "-f", "docker-compose.production.yml", "--env-file", ".env.docker", "ps", "--format", "json"],
            check=False
        )
    ...
    successful_snapshots = sum(
        1 for s in evidence["snapshots"]
        if all(h.get("status") == "ok" for h in s["health_checks"].values())
    )
    success_rate = (successful_snapshots / len(evidence["snapshots"]) * 100) if evidence["snapshots"] else 0
    
    print(f"Health check success rate: {success_rate:.1f}% ({successful_snapshots}/{len(evidence['snapshots'])})")
    print()
    print(f"Artifacts:")
