...
def run_hysteresis_lock():
    ...
    raw_trace = np.real(np.trace(
        np.array([
            [np.cos(LAMBDA_CRITICAL * np.pi), -np.sin(LAMBDA_CRITICAL * np.pi), 0, 0],
            [np.sin(LAMBDA_CRITICAL * np.pi), np.cos(LAMBDA_CRITICAL * np.pi), 0, 0],
            [0, 0, np.cos(LAMBDA_CRITICAL * np.pi), -np.sin(LAMBDA_CRITICAL * np.pi)],
            [0, 0, np.sin(LAMBDA_CRITICAL * np.pi), np.cos(LAMBDA_CRITICAL * np.pi)]
        ], dtype=complex)
        @ np.array([
            [np.cos(LAMBDA_CRITICAL * np.pi), -np.sin(LAMBDA_CRITICAL * np.pi), 0, 0],
            [np.sin(LAMBDA_CRITICAL * np.pi), np.cos(LAMBDA_CRITICAL * np.pi), 0, 0],
    ...
    engine = TopologicalHolonomyEngine(num_sites=1000, calibrated_scale=calibrated_scale)
    ...
    for i in range(100):  # Increased iterations for "Crystallization"
        ...
        if i % 10 == 0:
            logger.info(f"[PSO] Iteration {i}: Best Fit {best_fitness:.4f} at λ {best_lambda:.6f}")
    ...
    if final_chern == 1 and abs(final_action - target_action) < 0.01:
        ...
        logger.info(f"⚠️ Action Delta: {abs(final_action - target_action):.6f} (target: <0.01)")
        return False
