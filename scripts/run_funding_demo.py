...
class FundingDemoRunner:
        ...
        try:
            while elapsed < self.duration_seconds:
                ...
                self.metrics_history.append({
                    ...
                    'hashrate_efficiency': metrics.hashrate_current / max(metrics.hashrate_target, 1.0),
                ...
                if int(elapsed) % 60 == 0 or elapsed == 0:
                    ...
                    print(f"⏱️  Progress: {progress:.1f}% | {minutes_remaining:.1f} minutes remaining | "
    ...
    async def simulate_mining_iteration(self, salamander, mining_system) -> None:
        ...
        mining_system.controller['hashrate'] = max(100.0, min(200.0, base_hashrate + fluctuation))
    ...
    def generate_funding_report(self) -> Dict[str, Any]:
        ...
        if self.metrics_history:
            avg_hashrate = sum(m['hashrate_current'] for m in self.metrics_history) / len(self.metrics_history)
            avg_efficiency = sum(m['hashrate_efficiency'] for m in self.metrics_history) / len(self.metrics_history)
            avg_acceptance_rate = sum(m['share_acceptance_rate'] for m in self.metrics_history) / len(self.metrics_history)
            ...
            avg_hashrate = avg_efficiency = avg_acceptance_rate = max_hashrate = min_hashrate = 0.0
        ...
        report = {
            ...
            # Treasury state
            'treasury_state': salamander.get_treasury_state() if 'salamander' in locals() else {},
        ...
        # Save report to file
        report_file = Path(__file__).parent.parent / "artifacts" / f"funding_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        print(f"Evidence Integrity Seal: {self.salamander_core.audit_log.seal()[:16]}...")
