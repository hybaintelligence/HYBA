...
async def main():
    ...
    print(
        f"Authorized: {handshake.authorized}  extranonce1={handshake.extranonce1}  connect_ms={connect_ms}"
    )
    ...
    print(
        f"Job {current_job.job_id} | difficulty={difficulty} | prevhash={current_job.prevhash[:16]}..."
    )
    ...
    print(
        f"Searching {ncpus} workers × {chunk_size:,} nonces ({sprint_secs}s limit) ..."
    )
    t0 = time.time()
