...
def generate_su2_link(lambda_param):
    ...
    perturbation = np.random.normal(0, 0.001, (2, 2)) + 1j * np.random.normal(
        0, 0.001, (2, 2)
    )
    matrix += perturbation
...
def run_riemann_spectral_probe():
    ...
    logger.info(
        f"[Construction] Transfer matrix unitary check: {np.abs(np.linalg.det(T)):.6f}"
    )
    logger.info("")
    ...
    slope_g, intercept_g, r_value_g, p_val_g, std_err_g = stats.linregress(
        hist, gue_fit
    )
    ...
    try:
        ...
        try:
            ...
            if loop.is_running():
                ...
                logger.info(
                    "[Broadcast] Event loop already running, skipping async send"
                )
            else:
            ...
            logger.info(
                "[Broadcast] No event loop available, message prepared but not sent"
            )
