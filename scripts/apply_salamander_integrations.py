...
def verify_qaas_integration(main_py: Path) -> bool:
    ...
    checks = [
        ("quantum_as_a_service_execute_hardened imported", "quantum_as_a_service_execute_hardened" in content),
        ("public_computational_intelligence_service imported", "public_computational_intelligence_service" in content),
...
def verify_multi_agent_integration(reflexive_controller: Path) -> bool:
    ...
    checks = [
        ...
        ("orchestrator initialized in __init__", "self.orchestrator = SwarmOrchestrator()" in content),
...
def verify_pulvini_integration(mining_engine: Path) -> bool:
    ...
    checks = [
        ("PulviniCompressedQuantumSolver imported", "PulviniCompressedQuantumSolver" in content),
        ("PulviniCompressedQuantumSolver instantiated", "PulviniCompressedQuantumSolver(" in content),
        ("phi_folding_mathematical_proof imported", "phi_folding_mathematical_proof" in content),
    ]
...
def verify_memory_seed_integration(main_py: Path) -> bool:
    ...
    checks = [
        ...
        ("_load_memory_seed called in lifespan", "await _load_memory_seed(app)" in content),
...
def main():
    ...
    reflexive_controller = backend_root / "hyba_genesis_api" / "core" / "reflexive_controller.py"
    mining_engine = backend_root / "pythia_mining" / "phi_unified_mining_engine.py"
    
    if not all([main_py.exists(), reflexive_controller.exists(), mining_engine.exists()]):
    ...
    logger.info("-" * 80)
    multi_agent_ok, multi_agent_checks = verify_multi_agent_integration(reflexive_controller)
    results["multi_agent"] = {
    ...
    # Save report
    report_path = Path(__file__).parent.parent / "artifacts" / "integration_verification_report.json"
