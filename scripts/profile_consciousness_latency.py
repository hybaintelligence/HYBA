...
class LatencyProfiler:
    ...
    def profile_phi_measurement_loop(self, engine: ConsciousnessEngine, dim: int = 32) -> Dict[str, Any]:
        """
        ...
        for i, state in enumerate(states):
            ...
            if (i + 1) % 100 == 0:
                print(f"   Progress: {i+1}/{self.num_samples} iterations "
    ...
    def analyze_bottlenecks(self, top_n: int = 20) -> List[Tuple[str, float, int, float]]:
        """
        ...
        for func, (cc, nc, tt, ct, callers) in stats_dict.items():
            ...
            bottlenecks.append((func_name, cumulative_time_ms, call_count, time_per_call_ms))
    ...
    def classify_bottleneck_type(self, func_name: str) -> str:
        ...
        if any(op in func_lower for op in ['linalg', 'eigh', 'eigvalsh', 'dot', 'matmul', 'einsum']):
            return 'LINALG'
        elif any(op in func_lower for op in ['iit', 'phi_max', 'partition', 'emd', 'wasserstein']):
            return 'IIT'
        elif any(op in func_lower for op in ['array', 'asarray', 'copy', 'zeros', 'ones']):
            return 'MEMORY'
    ...
    def compute_amdahl_speedup(
        ...
        speedup = 1.0 / ((1 - bottleneck_fraction) + bottleneck_fraction / speedup_factor)
        return speedup
...
def profile_consciousness_latency(
    ...
    iit_analyzer = IIT4Analyzer(system_size=dimension, enhanced_partitioning=enhanced_iit)
    engine = ConsciousnessEngine(
        operator=operator,
        config=config,
        iit_analyzer=iit_analyzer
    )
    ...
    print("="*80)
    print(f"{'Rank':<6} {'Type':<10} {'Function':<50} {'Time(ms)':<12} {'Calls':<10} {'Per-Call(ms)':<12}")
    print("-" * 100)
    ...
    for rank, (func_name, cum_time, calls, per_call) in enumerate(bottlenecks, 1):
        ...
        print(f"{rank:<6} {func_type:<10} {display_name:<50} {cum_time:>11.2f} {calls:>9} {per_call:>11.4f}")
    ...
    for category, time_ms in sorted(bottleneck_categories.items(), key=lambda x: x[1], reverse=True):
    ...
    if bottleneck_categories:
        ...
        if dominant_category[0] == 'LINALG':
            ...
            print(f"   Amdahl's Law Projection (10x): {profiler.compute_amdahl_speedup(dominant_fraction, 10):.2f}x overall")
            print(f"   Amdahl's Law Projection (50x): {profiler.compute_amdahl_speedup(dominant_fraction, 50):.2f}x overall")
            print("\n   Implementation:")
            ...
            print(f"   Amdahl's Law Projection (5x): {profiler.compute_amdahl_speedup(dominant_fraction, 5):.2f}x overall")
            print(f"   Amdahl's Law Projection (20x): {profiler.compute_amdahl_speedup(dominant_fraction, 20):.2f}x overall")
            print("\n   Implementation:")
            ...
            print(f"   Amdahl's Law Projection (2x): {profiler.compute_amdahl_speedup(dominant_fraction, 2):.2f}x overall")
            print(f"   Amdahl's Law Projection (5x): {profiler.compute_amdahl_speedup(dominant_fraction, 5):.2f}x overall")
            print("\n   Implementation:")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--iterations', type=int, default=1000, help='Number of measurement cycles')
    parser.add_argument('--enhanced-iit', action='store_true', help='Use enhanced IIT partitioning')
