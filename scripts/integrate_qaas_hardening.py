#!/usr/bin/env python3
"""
QaaS Production Hardening Integration Script

Automatically integrates the production-hardened execute() method into quantum_as_a_service.py

This replaces the old execute method with the new version that includes:
- Per-computer lock with 5s timeout (409 on busy)
- Scoped idempotency with 24h TTL
- Enhanced evidence seals v2.0
- Execution failure handling without quota consumption

Run: python3 scripts/integrate_qaas_hardening.py
"""
...
HARDENED_EXECUTE = """    def execute(self, request: QuantumWorkloadRequest) -> Dict[str, Any]:
        ...
        # Scoped idempotency: customer:computer:idempotency_key with 24h TTL
...
def main():
    ...
    print(f"  - Added lines: {len(HARDENED_EXECUTE.split(chr(10)))}")
