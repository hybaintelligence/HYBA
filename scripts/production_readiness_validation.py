    ...
    def check(self, name: str, test_fn, expected: Optional[Any] = None) -> bool:
        ...
        try:
            ...
            self.checks.append({
                "name": name,
                "passed": passed,
                "result": result if isinstance(result, (str, int, float, bool)) else str(result)[:100],
...
def main() -> int:
    ...
    # Write report
    report_path = Path(__file__).resolve().parents[1] / "artifacts" / "production_validation_report.json"
