...
def _make_job(target: int) -> MiningJob:
    return MiningJob(
        job_id="pulvini-32", prevhash="00" * 32,
...
def main() -> None:
    ...
    for trial in range(args.trials):
        ...
        for r in results:
            ...
            print(f"  {r.name:<28} hits={r.total_hits:>4}  "
    ...
    for name in names:
        ...
        fhi_vals = [r.first_hit_iteration for t in all_results for r in t
    ...
    print(f"PULVINI 32 serial:         {p32s_hps/1000:.1f} kH/s  "
          f"({p32s_hps/bf_hps:.2f}x)")
