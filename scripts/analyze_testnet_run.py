...
def analyze_testnet_run(input_pattern: str, output_file: Path) -> int:
    ...
    pool_responses = list(
        data.get("pool_responses", data.get("pool_response_history", []))
    )
    target_evidence = data.get("target_evidence", data.get("target_selection", {}))
    proposals_accepted = int(
        data.get("proposals_accepted", data.get("accepted_proposals", 0))
    )
    circuit_breaker_trips = int(
        data.get(
            "circuit_breaker_trips", data.get("autonomous_circuit_breaker_trips", 0)
        )
    )
    ...
    analysis = {
        ...
        "pool_responses": {
            ...
            "accepted": sum(
                1 for response in pool_responses if response.get("accepted")
            ),
            "rejected": sum(
                1 for response in pool_responses if not response.get("accepted")
            ),
        },
        "autonomous_behavior": {
            "reflexive_cycles": int(
                data.get("reflexive_cycle_count", data.get("epochs", 0))
            ),
            "proposals_generated": int(
                data.get("proposals_generated", len(data.get("proposals", [])))
            ),
    ...
    analysis["readiness_verdict"] = (
        ...
        and proposals_accepted > 0
        and len(pool_responses) > 10
        else "NO_GO"
    )
    ...
    print(
        f"   Proposals accepted: {analysis['autonomous_behavior']['proposals_accepted']}"
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--input", dest="input_flag", help="Input JSON artifact or glob"
    )
    parser.add_argument(
        "--output", dest="output_flag", type=Path, help="Output analysis JSON"
    )
