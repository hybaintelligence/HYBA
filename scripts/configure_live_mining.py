...
Examples:
  ...
  python3 scripts/configure_live_mining.py --enable-live
    ...
    parser.add_argument(
        '--viabtc-pass',
        default='123',
        help='ViaBTC password (default: 123)'
    )
    ...
    parser.add_argument(
        '--generate-jwt',
        action='store_true',
        help='Generate and set new JWT secret'
    )
    
    parser.add_argument(
        '--jwt-secret',
        help='Use custom JWT secret (overrides --generate-jwt)'
    )
    ...
    print(f"✓ ViaBTC: {args.viabtc_user} (default)" if args.default_pool == 'viabtc' else f"✓ ViaBTC: {args.viabtc_user}")
    print(f"✓ Braiins: {args.braiins_user}" + (" (default)" if args.default_pool == 'braiins' else ""))
