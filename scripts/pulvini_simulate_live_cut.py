...
def run_live_cut_drill(nodes: list[int], *, min_purity: float, state_path: Path) -> dict[str, Any]:
    if IMPORT_ERROR is not None or PulviniOverlayConcentrator is None or PulviniAutonomicsEngine is None:
        raise RuntimeError(
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        payload = run_live_cut_drill(args.nodes, min_purity=args.min_purity, state_path=args.state)
    ...
    if args.json:
        ...
        status = "PASS" if payload["readiness"]["passed"] else "FAIL"
    ...
    return 0 if payload["readiness"]["passed"] else 1
