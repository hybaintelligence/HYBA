...
def features(rows: Sequence[Dict[str, str]], needle_quantile: float = 0.10) -> List[FeatureRecord]:
    ...
    for row in rows:
        ...
        merkle_ratio = hex_ratio(row.get("merkle_root", "")[:16]) if row.get("merkle_root", "") else 1.0
        component_distances = [
        ...
        phi_score = sum((INV_PHI ** i) * value for i, value in enumerate(component_distances, start=1))
        out.append(FeatureRecord(
            height=height,
...
def enrichment(rows: Sequence[FeatureRecord], k: int) -> Dict[str, float]:
    ...
    return {"hits": float(hits), "precision": hits / max(1, k), "recall": hits / max(1, needles)}

def baseline(rows: Sequence[FeatureRecord], k: int, trials: int = 2000, seed: int = 618033) -> Dict[str, float]:
    rng = random.Random(seed)
    values = []
    for _ in range(trials):
        sample = rng.sample(list(rows), min(k, len(rows)))
        values.append(sum(1 for r in sample if r.is_needle) / max(1, k))
    return {"precision_mean": statistics.mean(values), "precision_stdev": statistics.pstdev(values) if len(values) > 1 else 0.0}

def summary(rows: Sequence[FeatureRecord], ks: Sequence[int], trials: int) -> Dict[str, object]:
    payload: Dict[str, object] = {"sample_size": len(rows), "needle_count": sum(1 for r in rows if r.is_needle), "phi": PHI, "ks": {}}
    for raw_k in ks:
        ...
        z_score = 0.0 if sd == 0 else (phi_result["precision"] - random_result["precision_mean"]) / sd
        payload["ks"][str(k)] = {"phi_rank": phi_result, "random_baseline": random_result, "z_score": z_score}
    return payload
