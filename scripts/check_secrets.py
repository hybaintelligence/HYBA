...
PATTERNS = {
    ...
    "wallet_address_assignment": re.compile(r"(?i)(wallet|payout)_address\s*=\s*['\"](?:bc1|[13])[a-zA-HJ-NP-Z0-9]{25,62}['\"]"),
    "private_key_assignment": re.compile(r"(?i)(private_key|pool_password|jwt_secret|api_key_secret)\s*=\s*['\"][^'\"]{12,}['\"]"),
}
...
def main() -> int:
    ...
    if findings:
        ...
        print("\n".join(findings))
        return 1
    print("Secret hygiene scan passed: no credential patterns found in .py/.ts/.tsx files")
    return 0
