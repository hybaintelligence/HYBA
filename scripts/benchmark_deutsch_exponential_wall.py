...
def benchmark_state_vector_memory_scaling():
    ...
    for num_qubits in [4, 6, 8, 10, 12, 14]:
        ...
        print(f"Qubits: {num_qubits:2d} | State Size: {state_size:8d} | Memory: {memory_mb:10.2f} MB")
    ...
    for num_qubits in [20, 30, 40, 50]:
        ...
        print(f"Qubits: {num_qubits:2d} | State Size: {state_size:20d} | Memory: {memory_tb:15.2f} TB = {memory_pb:10.2f} PB")
...
def benchmark_computation_time_scaling():
    ...
    for num_qubits in [4, 6, 8, 10]:
        ...
        print(f"Qubits: {num_qubits:2d} | State Size: {state_size:8d} | Time: {avg_time*1000:10.4f} ms")
    ...
    for i in range(len(results) - 1):
        ...
        print(f"{results[i]['num_qubits']} -> {results[i+1]['num_qubits']} qubits: {ratio:.2f}x")
...
def benchmark_tensor_network_efficiency():
    ...
    for num_qubits in [50, 100, 200, 500, 1000]:
        ...
        compression_ratio = state_vector_memory_tb / (memory_mb / 1024) if memory_mb > 0 else float('inf')
        ...
        print(f"Qubits: {num_qubits:4d} | Params: {num_params:8d} | Memory: {memory_mb:8.2f} MB | Compression: {compression_ratio:.2e}x")
...
def benchmark_deutsch_prediction_unstructured():
    ...
    print(f"  Qubits: 30 | Bond: 4 | Params: {num_params_structured} | Entropy: {entropy_structured:.4f}")
    ...
    print(f"  Qubits: 30 | Bond: 64 | Params: {num_params_unstructured} | Entropy: {entropy_unstructured:.4f}")
    ...
    print("This confirms Deutsch's prediction: classical simulation is efficient only for structured states")
...
def run_all_benchmarks():
    ...
    print()
    print("3. Tensor networks achieve exponential compression ONLY for structured states")
    ...
    print()
    print("5. Unstructured states hit exponential wall (Deutsch's prediction confirmed)")
    ...
    print("- Tensor networks provide efficient approximation for structured states only")
    print("- This is a statement about classical approximation, not quantum computation")
    print()
