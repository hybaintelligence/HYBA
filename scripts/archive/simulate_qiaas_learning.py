...
def simulate_learning_cycle():
    ...
    print(
        f"   Knowledge explanations: {knowledge.get_knowledge_metrics()['total_explanations']}"
    )
    ...
    if outcome["accepted"]:
        explanation = knowledge.create_knowledge_from_success(
            strategy_id="yang_mills_phi_relationship", context=context, outcome=outcome
        )
    ...
    if outcome["accepted"]:
        explanation = knowledge.create_knowledge_from_success(
            strategy_id="phi_search_reduction", context=context, outcome=outcome
        )
        print(f"\n   ✅ SUCCESS: Intelligence learned!")
        print(f"   📝 Explanation: {explanation.explanation_text[:120]}...")
    ...
    print(
        f"   Predicted alternative outcome: {counterfactual.predicted_counterfactual_outcome}"
    )
    ...
    best_strategy = knowledge.best_explanation_for_context(
        {"problem": "yang_mills_mass_gap", "phi_relationship": True}
    )
    ...
    best_strategy = knowledge.best_explanation_for_context(
        {"problem": "p_vs_np", "phi_resonance": PHI}
    )
