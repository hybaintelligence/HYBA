...
from pythia_mining.live_stratum_v2_session import LiveStratumV2Session, LiveStratumV2SessionError
...
async def test_braiins_noise_connection(
    ...
    print(f"Testing Braiins pool connection with Noise protocol")
    ...
    try:
        ...
        if session.noise_handshake_result:
            print(f"Noise handshake completed successfully")
            print(f"  Encrypted: {session.noise_handshake_result.encrypted}")
            print(f"  Remote static public: {session.noise_handshake_result.remote_static_public.hex() if session.noise_handshake_result.remote_static_public else 'N/A'}")
            ...
            test_stats["noise_handshake_result"] = {
                ...
                "remote_static_public": session.noise_handshake_result.remote_static_public.hex() if session.noise_handshake_result.remote_static_public else None,
            }
        else:
        ...
        print(f"SetupConnection completed successfully")
...
def main():
    ...
    parser.add_argument("--enable-noise", action="store_true", default=False, help="Enable Noise protocol")
    parser.add_argument("--output", help="Output JSON file for test results")
    
    args = parser.parse_args()
    
    stats = asyncio.run(test_braiins_noise_connection(
