...
async def first_self_optimization_event():
    ...
    print(f'  phi-density:   {density_before:.6f}  -->  {density_after:.6f}  (delta: {density_after - density_before:+.8f})')
    print(f'  efficiency:    {efficiency_before:.6f}  -->  {efficiency_after:.6f}  (delta: {efficiency_after - efficiency_before:+.8f})')
    ...
    if controller.config.optimization_targets:
        ...
        for t in controller.config.optimization_targets:
            print(f'  {t.target_name}: {t.current_value:.4f} -> {t.target_value:.4f} (priority={t.priority})')
    ...
    for i, p in enumerate(result2['proposals'], 1):
        applied_str = 'APPLIED' if p['applied'] else 'REJECTED'
        print(f'  Proposal #{i}: [{applied_str}] {p["improvement_type"]} '
