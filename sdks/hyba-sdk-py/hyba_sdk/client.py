...
class QuantumFinanceNamespace:
    ...
    def portfolio_qaoa(self, portfolio: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        payload = {
            ...
            "query_type": "optimize",
            "context": {
                "capability_family": "quantum-finance",
                "portfolio": portfolio,
                **kwargs,
            },
        }
...
class HybaClient:
    ...
    def _request(self, method: str, path: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            raise HybaApiError(
                f"Request timed out after {self.timeout}s", status_code=504
            )
        except requests.exceptions.ConnectionError:
            raise HybaApiError("Failed to connect to HYBA API", status_code=503)
    ...
    def _handle_error_response(self, response: requests.Response) -> None:
        ...
        if response.status_code == 401:
            ...
            raise ServiceNotFoundError(
                service_id="unknown", message=message, response=error_data
            )
        elif response.status_code == 422:
            raise ValidationError(
                message, validation_errors=error_data.get("errors"), response=error_data
            )
        elif response.status_code == 429:
            retry_after = response.headers.get("Retry-After")
            if "quota" in message.lower():
                raise QuotaExceededError(
                    message, quota_info=error_data.get("quota"), response=error_data
                )
            else:
    ...
    def provision_service(
        ...
        **kwargs,
        ...
        response = self._request(
            "POST", "/api/v1/computational-intelligence-services", json=payload
        )
        ...
        return ComputationalIntelligenceService(
            client=self, service_id=service_id, **response
        )
    ...
    def list_services(self) -> List["ComputationalIntelligenceService"]:
        ...
        return [
            ComputationalIntelligenceService(client=self, **svc) for svc in response
        ]
    ...
    def get_service(self, service_id: str) -> "ComputationalIntelligenceService":
        ...
        """
        response = self._request(
            "GET", f"/api/v1/computational-intelligence-services/{service_id}"
        )
