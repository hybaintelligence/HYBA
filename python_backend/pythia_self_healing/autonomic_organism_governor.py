...
class ImmutableInvariantGuard:
    ...
    def assert_proposal_only(self, payload: Mapping[str, Any], *, context: str) -> None:
        ...
        if violations:
            raise AutonomicInvariantError(
                f"{context}: invariant violations detected: {violations}"
            )

    def find_violations(
        self, payload: Any, *, path: str = "$", violations: list[str] | None = None
    ) -> list[str]:
...
class OrganismSignal:
    ...
    def __post_init__(self) -> None:
        ...
        if not math.isfinite(self.pre_injury_phi) or self.pre_injury_phi < 0:
            raise AutonomicInvariantError(
                "pre_injury_phi must be finite and non-negative"
            )
        if not math.isfinite(self.post_recovery_fidelity) or not (
            0.0 <= self.post_recovery_fidelity <= 1.0
        ):
            raise AutonomicInvariantError(
                "post_recovery_fidelity must be finite and in [0, 1]"
            )
        if (
            not math.isfinite(self.recovery_duration_ms)
            or self.recovery_duration_ms < 0
        ):
            raise AutonomicInvariantError(
                "recovery_duration_ms must be finite and non-negative"
            )
        if not self.status:
    ...
    def from_event(cls, event: Any) -> "OrganismSignal":
        return cls(
            ...
            pre_injury_phi=_finite_float(
                getattr(event, "pre_injury_phi"), field_name="pre_injury_phi"
            ),
            post_recovery_fidelity=_finite_float(
                getattr(event, "post_recovery_fidelity"),
                field_name="post_recovery_fidelity",
            ),
            scarring_detected=bool(getattr(event, "scarring_detected")),
            recovery_duration_ms=_finite_float(
                getattr(event, "recovery_duration_ms"),
                field_name="recovery_duration_ms",
            ),
...
class CrossLaneRegenerationIntelligence:
    ...
    def analyse(
        ...
        *,
        ...
        signal_list = sorted(
            list(signals),
            key=lambda signal: (signal.lane_id, signal.module_id, signal.status),
        )
        phi_floor = _finite_float(phi_floor, field_name="phi_floor")
        scar_free_fidelity_floor = _finite_float(
            scar_free_fidelity_floor, field_name="scar_free_fidelity_floor"
        )
        ...
        scarring_lanes = sorted(
            {signal.lane_id for signal in signal_list if signal.scarring_detected}
        )
        low_fidelity = [
            signal
        ...
        phi_floor_lanes = sorted(
            {
                signal.lane_id
                for signal in signal_list
                if signal.pre_injury_phi <= phi_floor
            }
        )
        ...
        if len(scarring_lanes) >= 2:
            findings.append(
                CrossLaneFinding(
                    ...
                    evidence={
                        "count": len(scarring_lanes),
                        "scar_free_fidelity_floor": scar_free_fidelity_floor,
                    },
        ...
        cross_lane_phi_density = self._cross_lane_phi_density(
            signal_list, phi_floor=phi_floor
        )
        ...
        report = {
            ...
            "fidelity_dispersion": (
                pstdev(fidelity_values) if len(fidelity_values) > 1 else 0.0
            ),
            "cross_lane_phi_density": cross_lane_phi_density,
            "findings": [asdict(finding) for finding in findings],
            "multi_limb_regeneration_sequence": multi_limb_sequence,
            "collapse_validation_scope": (
                "multi_lane" if len(scarring_lanes) >= 2 else "single_lane"
            ),
    ...
    @staticmethod
    def _cross_lane_phi_density(
        signals: list[OrganismSignal], *, phi_floor: float
    ) -> dict[str, Any]:
        if not signals:
            return {
                "mean_phi": 1.0,
    ...
    def _multi_limb_sequence(
        ...
        ordered = sorted(
            low_fidelity,
            key=lambda signal: (
                signal.post_recovery_fidelity,
                -signal.recovery_duration_ms,
                signal.lane_id,
            ),
        )
        return [
...
class TemporalRegenerationMemory:
    ...
    def record(
        self, packet: Mapping[str, Any], *, event: OrganismSignal | None = None
    ) -> dict[str, Any]:
    ...
    def summarise(self) -> dict[str, Any]:
        ...
        recurring_lanes = sorted(
            lane for lane, count in lane_counts.items() if count >= 2
        )
        recurring_targets = sorted(
            target for target, count in target_counts.items() if count >= 2
        )
        return {
            "record_count": len(self._records),
            "chain_head": (
                self._records[-1]["chain_hash"] if self._records else "GENESIS"
            ),
    ...
    def validate_chain(self) -> bool:
        ...
        for record in self._records:
            ...
            if _sha256(clone) != chain_hash:
                return False
            if (
                record.get("auto_apply") is not False
                or record.get("sovereign_human_gate") is not True
            ):
                return False
            previous = chain_hash
        return True
...
class PredictiveRegenerationEngine:
    ...
    def forecast(
        ...
        *,
        ...
        finding_types = {
            finding["finding_type"] for finding in clri_report.get("findings", [])
        }
        ...
        for signal in signal_list:
            ...
            drivers: list[str] = []
            fidelity_gap = max(
                0.0, scar_free_fidelity_floor - signal.post_recovery_fidelity
            )
            if fidelity_gap > 0:
        ...
        report = {
            ...
            "status": (
                "PRE_DAMAGE_PROPOSALS_AVAILABLE"
                if forecast_items
                else "NO_PRE_DAMAGE_SIGNAL"
            ),
            "predictions": forecast_items,
            "sovereign_human_gate": True,
            "auto_apply": False,
        }
        self.guard.assert_proposal_only(
            report, context="predictive regeneration report"
        )
        return report
    ...
    def stage_plan(
        ...
        *,
        ...
        if "phi_floor_breach" in finding_types or len(recurring_lanes) >= 8:
            ...
            "correlated_scarring" in finding_types
            or prediction_count >= 2
            or len(recurring_lanes) >= 2
        ):
            tier = "REGIONAL_ORGAN"
...
class PulviniAutonomicHandshake:
    ...
    def stage(
        ...
        *,
        ...
        unsafe_candidates = [
            candidate
            for candidate in state.get("algorithmic_alternatives", [])
            if isinstance(candidate, Mapping) and candidate.get("auto_apply") is True
        ]
        if unsafe_candidates:
            raise AutonomicInvariantError(
                "Pulvini candidate exchange attempted auto-apply"
            )
        report = {
...
class RegulatorEvidenceEngine:
    ...
    def seal(self, report: Mapping[str, Any]) -> dict[str, Any]:
        ...
        payload = {
            ...
            "chain_of_custody": {
                ...
                "temporal_memory_head": report.get("temporal_memory", {}).get(
                    "chain_head"
                ),
                "rewiring_plan": report.get("hierarchical_rewiring", {}).get("plan_id"),
                "benchmark_evolution": report.get("benchmark_evolution", {}).get(
                    "benchmark_evolution_id"
                ),
            },
        }
    ...
    def verify(
        self, sealed_payload: Mapping[str, Any], report: Mapping[str, Any]
    ) -> bool:
...
class SalamanderOrganismGovernor:
    ...
    def __init__(
        self,
        temporal_memory: TemporalRegenerationMemory | None = None,
        *,
        guard: ImmutableInvariantGuard | None = None,
    ) -> None:
        self.guard = guard or ImmutableInvariantGuard()
    ...
    def analyse_manager(
        ...
        *,
        ...
        signals = [
            OrganismSignal.from_event(event)
            for event in getattr(manager, "fidelity_history", [])
        ]
        phi_floor = (
            float(getattr(manager, "PHI_FLOOR", 0.45))
            if hasattr(manager, "PHI_FLOOR")
            else 0.45
        )
        scar_floor = 0.999
        ...
        clri_report = self.clri.analyse(
            signals, phi_floor=phi_floor, scar_free_fidelity_floor=scar_floor
        )
        ...
        pulvini_handshake = self.pulvini.stage(
            clri_report=clri_report, pulvini_state=pulvini_state
        )
