...
def run_jpmorgan_demo(
    ...
    print(
        f"• PULVINI Working Set Compression: {post_quantum_results['pulvini']['working_set_compression_ratio']:.2f}x"
    )
    print(
        f"• PULVINI Retained State Compression: {post_quantum_results['pulvini']['retained_state_compression_ratio']:.2f}x (lossless)"
    )
    print(
        f"• POST-QUANTUM Advantage: {comparison['comparison']['pulvini_advantage']:.2f}x vs gzip"
    )
    print(
        f"• Salamander Audit Entries: {len(post_quantum_results['audit_log'].entries())}"
    )
...
def run_all_demos(
    ...
    # JPMorgan Demo
    try:
        jpmorgan_results = run_jpmorgan_demo(
