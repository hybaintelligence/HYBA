...
class PSOParticle:
    ...
    def update_velocity(self, global_best: str, global_best_fitness: float, w: float = 0.7, c1: float = 1.5, c2: float = 1.5):
        """
...
class PSOTaskAllocator:
    ...
    async def allocate_tasks(
        ...
        for task in tasks:
            ...
            particles = [PSOParticle(task_id, agents) for _ in range(self.particle_count)]
...
class SwarmTaskCoordinator:
    ...
    def learn_from_execution(self, task_id: str, agent_name: str, success: bool):
        ...
        self.agent_fitness[agent_name] = max(0.0, min(1.0, current_fitness + adjustment))
