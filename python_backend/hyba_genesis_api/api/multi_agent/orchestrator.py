...
class SalamanderOrchestrator:
    ...
    async def handle_regeneration(
        ...
        try:
            ...
            diagnosis_result = await self._execute_step(
                regeneration_id, "diagnosis", prompt, target_files, context
            )
            ...
            if verification_result["data"].get("passed", False):
                ...
                if execution_result["status"] == "completed":
                    ...
                    return {
                        ...
                        "status": "success",
                        "steps": self.active_tasks.get(regeneration_id, {}).get(
                            "steps", []
                        ),
                        "diagnosis": diagnosis_result["data"],
                        "plan": planning_result["data"],
                ...
                return await self._handle_verification_failure(
                    regeneration_id, verification_result, specialist_results
                )
    ...
    async def _execute_step(
        ...
        try:
            result = await asyncio.wait_for(
                agent.execute_task(task), timeout=self.task_timeout_seconds
            )
    ...
    async def _delegate_to_specialists(
        ...
        for subtask in subtasks:
            ...
            try:
                ...
                await self._broadcast_specialist_update(
                    regeneration_id, agent_name, result
                )
    ...
    def _handle_failure(
        self, regeneration_id: str, failure_result: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if regeneration_id in self.active_tasks:
            ...
            self.active_tasks[regeneration_id]["end_time"] = time.time()
            self.active_tasks[regeneration_id]["error"] = failure_result.get(
                "error", "Unknown error"
            )
    ...
    async def _broadcast_step_update(
        self, regeneration_id: str, step_result: Dict[str, Any]
    ):
        """
    ...
    async def _broadcast_specialist_update(
        self, regeneration_id: str, agent_name: str, result
    ):
        """
    ...
    def get_task_status(self, regeneration_id: str) -> Optional[Dict[str, Any]]:
        ...
        for task in reversed(self.task_history):
            if (
                task.get("prompt") == regeneration_id
                or task.get("regeneration_id") == regeneration_id
            ):
                return task
