...
class SwarmMessage(BaseModel):
    ...
    timestamp: float
    message_type: Literal["task", "result", "proposal", "vote", "pheromone", "alert", "consensus"]
...
class SwarmCommunication:
    ...
    async def receive(self, agent_name: str, timeout: float = 5.0) -> Optional[SwarmMessage]:
        """Receive a message for a specific agent."""
        if agent_name not in self.agents:
            return None
        
        try:
            return await asyncio.wait_for(self.agents[agent_name].get(), timeout=timeout)
        except asyncio.TimeoutError:
            return None
    ...
    def leave_pheromone(self, pattern_key: str, strength: float = 1.0):
        ...
        self.pheromone_trails[pattern_key] = self.pheromone_trails.get(pattern_key, 0) + strength
...
class SwarmEnabledAgent:
    ...
    async def send_message(self, receiver: str, message_type: str, payload: Dict, confidence: float = 0.7):
    ...
    async def propose_fix(self, diagnosis: Dict, fix_plan: Dict, confidence: float = 0.7) -> str:
        ...
        proposal_id = await self.comm.broadcast_proposal(proposal, self.name, confidence)
        self.proposal_history.append(proposal_id)
        
        return proposal_id
    
    async def vote_on_proposal(self, proposal_id: str, vote: Literal["approve", "reject", "abstain"], confidence: float = 0.7):
        """Vote on a proposal."""
