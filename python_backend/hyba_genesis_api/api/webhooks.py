...
class WebhookSubscription(BaseModel):
    ...
    events: List[WebhookEventType] = Field(
        ..., description="List of events to subscribe to", min_items=1
    )
    secret: str = Field(
        ...,
    ...
    timeout_seconds: int = Field(
        default=30, description="Timeout for webhook delivery", ge=5, le=300
    )
...
class WebhookDeliveryService:
    ...
    async def deliver_webhook(
        self, subscription: WebhookSubscription, event: WebhookEvent, attempt: int = 1
    ) -> WebhookDeliveryLog:
        """
        ...
        signature = self.generate_signature(subscription.secret, payload, timestamp)
        ...
        try:
            async with httpx.AsyncClient(
                ...
                response = await client.post(
                    str(subscription.url), headers=headers, content=payload
                )
                ...
                if response.status_code >= 500 or response.status_code == 429:
                    if attempt < subscription.retry_max_attempts:
                        backoff = subscription.retry_backoff_seconds * (
                            2 ** (attempt - 1)
                        )
    ...
    async def broadcast_event(
        ...
        for sub in subscriptions:
            ...
            task = asyncio.create_task(self.deliver_webhook(sub, event))
            tasks.append(task)
...
async def get_webhook_deliveries(
    webhook_id: Optional[str] = None, event_type: Optional[str] = None, limit: int = 100
) -> List[Dict[str, Any]]:
    """
            ...
            logs.append(
                {
                    ...
                    "next_retry": (
                        log.next_retry.isoformat() if log.next_retry else None
                    ),
                }
            )
