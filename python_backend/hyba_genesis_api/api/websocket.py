...
async def websocket_telemetry(websocket: WebSocket):
    ...
    try:
        logger.info(
            f"WebSocket client connected. Active connections: {len(active_connections)}"
        )
            ...
            try:
                ...
                try:
                    await websocket.send_json(
                        {"error": "Telemetry fetch failed", "status": "degraded"}
                    )
                except:
        ...
        logger.info(
            f"WebSocket client removed. Active connections: {len(active_connections)}"
        )
