...
class PoolCredentialRequest(BaseModel):
    ...
    def validate_plain_identifier(cls, value: Optional[str]) -> Optional[str]:
        ...
        if not all(char.isalnum() or char in "._-" for char in value):
            raise ValueError("identifier can only contain alphanumeric characters, dots, underscores, and hyphens")
        return value
...
async def require_mining_control(payload: TokenPayload = Depends(get_token_payload)) -> TokenPayload:
    if not any(role in MINING_CONTROL_ROLES for role in payload.roles):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=f"Mining control requires one of: {', '.join(sorted(MINING_CONTROL_ROLES))}")
    return payload
...
async def require_mining_read(payload: TokenPayload = Depends(get_token_payload)) -> TokenPayload:
    if not any(role in MINING_READ_ROLES for role in payload.roles):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=f"Mining read access requires one of: {', '.join(sorted(MINING_READ_ROLES))}")
    return payload
...
def _idempotency_key(request: Request, explicit: str | None, operation_type: str, parameters: dict[str, Any]) -> str:
    return explicit or request.headers.get("idempotency-key") or request.headers.get("x-idempotency-key") or mining_request_tracker.generate_idempotency_key(operation_type, parameters)
...
def _pool_config_response(config: PoolCredentialConfig, active_pool_id: Optional[str]) -> Dict[str, Any]:
    ...
    payload["status"] = "connected" if active_pool_id == config.pool_id else "configured" if payload.get("configured") else "not_configured"
    return payload
...
def _resolve_connect_config(req: ConnectRequest) -> PoolCredentialConfig:
    ...
    if any([req.worker, req.password, req.username, req.btc_address, req.nicehash_pool_id, req.url]):
        config = PoolCredentialConfig(
            ...
            username=req.username or (req.worker if req.pool_id in {"viabtc", "braiins"} else "") or config.username,
            password=req.password or config.password,
            btc_address=req.btc_address or (req.worker if req.pool_id == "ckpool" else "") or config.btc_address,
...
async def configure_pool(req: PoolCredentialRequest, request: Request, idempotency_key: str | None = Header(None, alias="Idempotency-Key")):
    ...
    try:
        ...
        result = {"status": "configured", "request_id": request_id, "tracked_request_id": tracked.request_id, "idempotency_key": idem, "pool": _pool_config_response(config, _ACTIVE_CONNECTION.get("pool_id") if _ACTIVE_CONNECTION else None)}
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail={"error": "invalid_pool_config", "detail": str(exc), "request_id": request_id}) from exc
    finally:
    ...
    for config in configs.values():
        ...
        pool_payload.update({
            ...
            "performance": {
                ...
                "acceptance_rate": shares["accepted"] / max(shares["submitted"], 1) if is_active else 0,
            },
        })
...
async def connect_to_pool(req: ConnectRequest, request: Request, idempotency_key: str | None = Header(None, alias="Idempotency-Key")):
    ...
    try:
        ...
        if _ACTIVE_CONNECTION is not None:
            if not req.switch:
                raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail={"error": "mining_already_connected", "active_connection": _redacted_connection(_ACTIVE_CONNECTION)})
            stop_pythia_daemon()
            _ACTIVE_CONNECTION = None
            if midas_state_machine.get_state() == MiningState.RUNNING:
                midas_state_machine.transition(MiningState.STOPPING, request_id=request_id, reason="operator requested pool switch")
                midas_state_machine.transition(MiningState.STOPPED, request_id=request_id, reason="previous pool stopped before switch")
...
async def switch_pool(req: ConnectRequest, request: Request, idempotency_key: str | None = Header(None, alias="Idempotency-Key")):
    req.switch = True
    return await connect_to_pool(req, request, idempotency_key)


@router.post("/submit")
async def submit_job(job: SubmitJobRequest, request: Request, _payload: TokenPayload = Depends(require_mining_control), idempotency_key: str | None = Header(None, alias="Idempotency-Key")):
...
async def mining_status():
    ...
    return {
        ...
        "telemetry_source": "live_api",
        "midas": {"state": midas_state_machine.get_state().value, "validation": midas_state_machine.validate_state_machine(), "rate_limiter": midas_rate_limiter.metrics(), "backpressure": midas_backpressure_guard.metrics(), "requests": mining_request_tracker.get_stats()},
        "timestamp": datetime.utcnow().isoformat(),
    }
...
async def disconnect(request: Request, idempotency_key: str | None = Header(None, alias="Idempotency-Key")):
    ...
    try:
        ...
        result = {"status": "disconnected", "request_id": request_id, "tracked_request_id": tracked.request_id, "idempotency_key": idem, "previous_pool": pool_name, "daemon": daemon_status, "midas_state": midas_state_machine.get_state().value}
        mining_request_tracker.update_request_status(tracked.request_id, RequestStatus.COMPLETED, result=result)
        return result
