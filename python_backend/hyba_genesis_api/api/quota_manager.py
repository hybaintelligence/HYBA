...
class QuotaManager:
    ...
    def refund_quota(
        self, customer_id: str, units: int, execution_id: str, reason: str
    ) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "utilization_percent": round(
                    (consumed / total * 100) if total > 0 else 0, 2
                ),
                "statistics": {
                    "total_executions": len(consumptions),
                    "total_refunds": len(refunds),
                    "refund_rate": round(
                        (len(refunds) / len(consumptions) * 100) if consumptions else 0,
                        2,
                    ),
                },
            }
