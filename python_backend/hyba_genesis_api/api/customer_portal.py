...
async def create_payment_method(
    ...
    tenant["payment_methods"].append(method)
    _save_store(store)
    return {
        "payment_method_id": payment_method_id,
        "provider": request.provider,
...
# ── Direct authenticated/self-service endpoints ─────────────────────────────
...
async def get_usage_history(months: int = 6, request: Request = None):
    tenant_id = "default"
...
async def get_quota_alert_config(request: Request):
    customer_id = request.headers.get("X-HYBA-Tenant-ID") or request.headers.get("X-HYBA-Customer-ID") or "default"
    conn = _safe_db()
    cursor = conn.cursor()
    cursor.execute(
...
async def get_invoices(limit: int = 12, request: Request = None):
    customer_id = "default"
    if request is not None:
        customer_id = request.headers.get("X-HYBA-Tenant-ID") or request.headers.get("X-HYBA-Customer-ID") or "default"
    conn = _safe_db()
