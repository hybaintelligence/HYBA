...
class TopologicalHolonomyEngine:
    ...
    def __init__(
        ...
        self.phi_generator = PhiEntropyGenerator(seed=phi_seed, memory_size=2**32)

        # Initialize MPS state
        self.mps = MPS(num_sites=num_sites, physical_dim=2, max_bond_dim=max_bond_dim)
    ...
    def parameterized_state(self, lambda_param: float) -> MPS:
        ...
        for i in range(self.num_sites):
            ...
            if self.haldane_twist > 0:
                ...
                haldane_phase = (
                    self.haldane_twist
                    * math.sin(2 * math.pi * lambda_param)
                    * (1 if i % 2 == 0 else -1)
                )
                theta += haldane_phase
    ...
    def compute_chern_number(
        self, num_points: int = 100, use_phi_sampling: bool = True
    ) -> ChernNumber:
...
def run_topological_holonomy_demonstration():
    ...
    holonomy = engine.parallel_transport_holonomy(
        path_type=HolonomyPathType.CLOSED_LOOP, num_steps=100, use_sld_gradient=True
    )
    ...
    for twist in twist_values:
        ...
        transition_scan = engine_twisted.scan_chern_transition(
            lambda_start=0.0, lambda_end=1.0, num_points=30
        )
    ...
    for result in transition_results:
        if result["transition_detected"]:
            print(
                f"✓ Twist {result['twist']:.2f}: Chern transition at λ={result['critical_lambda']:.4f}"
            )
            print(f"  Chern {result['chern_before']:.2f} → {result['chern_after']:.2f}")
        else:
            print(
                f"○ Twist {result['twist']:.2f}: No transition (Chern={result['final_chern']:.6f})"
            )
