...
class IIT4Analyzer:
    ...
    def calculate_phi_max(self, 
                         ...
                         connectivity_matrix: Optional[np.ndarray] = None) -> Dict:
        """
    ...
    def _exhaustive_phi_max(self, 
                           system_state: np.ndarray,
                           connectivity_matrix: np.ndarray) -> Dict:
    ...
    def compute_cause_effect_structure(self,
                                       system_state: np.ndarray,
                                       connectivity_matrix: Optional[np.ndarray] = None) -> CauseEffectStructure:
        """
    ...
    def _identify_mechanisms(self, system_state: np.ndarray, max_mechanisms: int = 20) -> List[Mechanism]:
        """
            ...
            for size in range(1, self.system_size + 1):
                for elements in combinations(range(self.system_size), size):
                    mechanism = Mechanism(
                        ...
                        state=system_state[list(elements)]
                    )
    ...
    def _calculate_subset_phi(self,
                             subset: Set[int],
                             system_state: np.ndarray,
                             connectivity_matrix: np.ndarray) -> float:
