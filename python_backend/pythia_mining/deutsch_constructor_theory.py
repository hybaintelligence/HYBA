...
from typing import Dict, List, Set, Tuple, Optional, Any, FrozenSet, Callable
...
class DeutschConstructorTheory:
    ...
    def analyze_reachability(
        ...
        return {
            ...
            "shortest_path_length": min(len(p) for p in reachable_paths) if reachable_paths else None,
    ...
    def verify_universal_constructor(
        ...
        if constructor_id not in self.registered_constructors:
            return {
                "is_universal": False,
                "reason": "Constructor not registered"
            }
        ...
        return {
            ...
            "incapable_tasks": [
                ...
                if not constructor.can_perform(task)
            ]
        }
        ...
        return ImpossibilityProof(
            ...
            proof_details={
                ...
                "violated_constraints": [
                    resource for resource in required_resources
                    if required_resources[resource] > available_resources.get(resource, float('inf'))
                ]
            }
        )
    ...
    def analyze_constructor_space(
        ...
        if self.impossibility_proofs:
            impossible_ratio = sum(
                1 for proof in self.impossibility_proofs.values()
