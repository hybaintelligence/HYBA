...
class IIT4ConceptualIntegration:
    ...
    def compute_conceptual_phi(
        ...
        cause_phi = self._compute_cause_phi(mechanism_state, connectivity_matrix[elements][:, elements])
        
        # Compute effect repertoire (simplified)
        effect_phi = self._compute_effect_phi(mechanism_state, connectivity_matrix[elements][:, elements])
    ...
    def build_complete_conceptual_structure(
        ...
        mechanisms = self.generate_conceptual_mechanisms(system_state, max_depth=self.max_concept_depth)
        ...
        cause_effect_structure = {
            ...
            "star_phi": star_phi,
            "average_conceptual_phi": np.mean([c.conceptual_phi for c in concepts]) if concepts else 0.0
        }
