        ...
        if self.noise_handshake:
            result["noise_handshake"] = {
                ...
                "remote_static_public": (
                    self.noise_handshake.remote_static_public.hex()
                    if self.noise_handshake.remote_static_public
                    else None
                ),
            }
        return result
        ...
        *,
        ...
        if self.enable_noise and NOISE_AVAILABLE:
            ...
            if remote_key is None and extract_pool_authority_key:
                ...
                if remote_key:
                    logger.info("Extracted pool authority key from URL")

            self.noise_wrapper.initialize(
                local_static_key=noise_static_key, remote_static_public=remote_key
            )
    ...
    async def _read_frame(self, *, timeout: Optional[float] = None) -> StratumV2Frame:
        ...
        if hasattr(self.transport, "read_frame"):
            frame = await asyncio.wait_for(
                self.transport.read_frame(timeout=timeout), timeout=read_timeout
            )
        else:
            header = await asyncio.wait_for(
                self.transport.read_exactly(SV2_HEADER_SIZE, timeout=timeout),
                timeout=read_timeout,
            )
            payload_length = decode_u24_le(header[3:6])
            payload = (
                await asyncio.wait_for(
                    self.transport.read_exactly(payload_length, timeout=timeout),
                    timeout=read_timeout,
                )
                if payload_length
                else b""
        ...
        if frame is None:
            raise LiveStratumV2SessionError(
                "SetupConnection response timeout: no Stratum V2 frame received"
            )
