...
class EnhancedBenchmarkSuite:
    ...
    def benchmark_grover_search_statistical(
        ...
        print(f"    95% CI: [{time_summary.ci_95_lower:.4f}, {time_summary.ci_95_upper:.4f}]")
        print(f"  Success Rate:")
        print(f"    Mean: {success_summary.mean*100:.2f}% ± {success_summary.std*100:.2f}%")
        print(f"  QOps/s:")
    ...
    def benchmark_error_correction_scaling(
        ...
        log_suppressions = np.log([r['suppression_factor'] for r in results if r['suppression_factor'] != float('inf')])
        
        if len(log_suppressions) > 1:
            slope, intercept, r_value, p_value, std_err = stats.linregress(
                log_distances[:len(log_suppressions)],
                log_suppressions
            )
    ...
    def benchmark_phi_resonance_correlation(
        self,
        n_samples: int = 1000
    ) -> Dict:
        """
        ...
        for i in range(n_samples):
            ...
            phi_alignment = sum(b * w for b, w in zip(bits, phi_weights)) / sum(phi_weights)
            
            # Simulate success (higher φ-alignment → higher success probability)
            success_threshold = 0.5 + 0.4565 * phi_alignment  # 0.5 baseline + 0.4565 from empirical 95.65%
        ...
        if high_phi_total > 0:
            ...
            binom_test = stats.binomtest(int(high_phi_successes), int(high_phi_total), 0.5, alternative='greater')
        ...
        print(f"  p-value: {p_value:.2e}")
        print(f"  High φ-alignment (>{PHI_INV:.3f}) success rate: {high_phi_rate*100:.2f}%")
    ...
    def benchmark_hardware_scaling(
        self,
        qubit_range: List[int] = [12, 16, 20, 24, 28]
    ) -> Dict:
        """
        ...
        for n_qubits in qubit_range:
            ...
            print(f"  {n_qubits} qubits: {elapsed:.4f}s, {compressed_memory_mb:.2f}MB (compressed)")
    ...
    def run_full_suite(self) -> Dict:
        ...
        suite_results['benchmarks']['error_correction_scaling'] = \
        ...
        suite_results['benchmarks']['hardware_scaling'] = \
