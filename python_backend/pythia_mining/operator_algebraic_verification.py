...
class CStarAlgebraCertificate:
    ...
    def verify_assertions(self) -> None:
        ...
        if not self.c_star_identity_holds:
            issues.append(
                f"C*-identity violated (error={self.c_star_identity_error:.2e})"
            )
...
class CStarAlgebraVerifier:
    ...
    def verify_operator(
        self, operator: np.ndarray, name: str = "unnamed_operator"
    ...
    def verify_state(
        self, rho: np.ndarray, name: str = "unnamed_state"
    ) -> StateCertificate:
        ...
        try:
            ...
            return StateCertificate(
                state_name=name,
                dimension=n,
                trace=round(trace, 12) if math.isfinite(trace) else float("nan"),
                is_positive=False,
                is_hermitian=is_hermitian,
                is_normalized=False,
                purity=float("nan"),
    ...
    def verify_channel(
        self, kraus_operators: Sequence[np.ndarray], name: str = "unnamed_channel"
    ) -> ChannelCertificate:
        ...
        if not ops:
            return ChannelCertificate(
                ...
                input_dim=0,
                output_dim=0,
                choi_positive_semidefinite=True,
    ...
    def verify_density_matrix_batch(
        ...
        return {
            name: self.verify_state(mat, name=name) for name, mat in matrices.items()
        }
    ...
    def verify_operator_batch(
        ...
        return {
            name: self.verify_operator(op, name=name) for name, op in operators.items()
        }
...
def phi_weighted_spectral_gap(operator: np.ndarray) -> Dict[str, Any]:
    ...
    gap = (
        float(sorted_abs[0] - sorted_abs[1])
        if len(sorted_abs) > 1
        else float(sorted_abs[0])
    )
...
def verify_lean4_proof_structure(
    theorem_name: str, hypotheses: List[str], conclusion: str, proof_steps: List[str]
) -> Dict[str, Any]:
    return {
        ...
        "proof_steps": [
            {"step": i + 1, "statement": s} for i, s in enumerate(proof_steps)
        ],
...
def verify_gelfand_naimark_theorem(
    ...
    for i in range(len(matrices)):
        for j in range(i + 1, len(matrices)):
            error = float(
                np.linalg.norm(
                    matrices[i] @ matrices[j] - matrices[j] @ matrices[i], "fro"
                )
            )
