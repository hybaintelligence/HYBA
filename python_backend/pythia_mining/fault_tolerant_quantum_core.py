...
class FaultTolerantQuantumCore:
    ...
    def _minimum_weight_pairing(
        self, defect_locations: List[Tuple[int, int, int]]
    ) -> float:
        """
        ...
        if len(defect_locations) > 18:
            ...
            while len(remaining) > 1:
                ...
                partner_idx, partner = min(
                    enumerate(remaining),
                    key=lambda item: abs(defect[1] - item[1][1])
                    + abs(defect[2] - item[1][2]),
                )
                total += float(
                    abs(defect[1] - partner[1]) + abs(defect[2] - partner[2])
                )
                remaining.pop(partner_idx)
            if remaining:
                _, row, col = remaining[0]
                total += float(
                    min(row + 1, col + 1, self.d - row - 1, self.d - col - 1)
                )
            return total
        ...
        for i, (_, row_i, col_i) in enumerate(defect_locations):
            boundary_costs[i] = float(
                min(row_i + 1, col_i + 1, self.d - row_i - 1, self.d - col_i - 1)
            )
