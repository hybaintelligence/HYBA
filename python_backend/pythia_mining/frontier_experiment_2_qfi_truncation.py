...
def measure_ground_state_energy(mps: MPS) -> float:
    ...
    for i in range(mps.num_sites - 1):
        # σ_z^i σ_z^{i+1} term
        zz_operator = np.kron(sigma_z, sigma_z)
...
def run_comparative_truncation_benchmark(
    ...
    print("=" * 80)
    print()
    print(f"Hypothesis: QFI-weighted truncation preserves energy better than SVD alone")
    ...
    print(f"✓ Standard: E_error = {standard_metrics.energy_error:.6f}, "
    ...
    print(f"✓ QFI-adaptive: E_error = {qfi_metrics.energy_error:.6f}, "
          f"fidelity = {fidelity_qfi:.4f}")
    print()
    ...
    entanglement_ratio = qfi_metrics.entanglement_error / (standard_metrics.entanglement_error + EPSILON)
    ...
    print(f"  → {'✓ QFI preserves energy BETTER' if error_ratio < 1.0 else '✗ Standard preserves energy better'}")
    print()
