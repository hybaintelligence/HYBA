...
class ModuleInspection:
    ...
    def entropy_score(self) -> float:
        return float(
            self.lines + 5 * self.functions + 8 * self.classes + 3 * len(self.imports)
        )
...
def _edge_weight(source: ModuleInspection, target: ModuleInspection) -> float:
    ...
    import_affinity = (
        0.25
        if target.name in source.imports or target.name.split(".")[-1] in source.imports
        else 0.0
    )
    invariant_affinity = min(0.55, 0.11 * len(shared_invariants))
    entropy_balance = 1.0 - min(
        abs(source.entropy_score - target.entropy_score) / 2_000.0, 0.30
    )
    return round(
        max(
            0.10,
            min(
                0.99,
                0.20 + import_affinity + invariant_affinity + 0.20 * entropy_balance,
            ),
        ),
        3,
    )
...
def _build_edges(modules: list[ModuleInspection]) -> list[tuple[str, str, float]]:
    ...
    for module in modules:
        for imported in module.imports:
            candidate = module_by_name.get(imported) or module_by_leaf.get(
                imported.split(".")[-1]
            )
...
def build_runtime_surroundings(
    controller: Any, package_root: Path | None = None
) -> Any:
    ...
    entropy_sources = [
        ...
        for module in sorted(
            inspections, key=lambda item: item.entropy_score, reverse=True
        )[:12]
    ]
    stable_core = [
        module.name
        for module in inspections
        if any(hint in module.name for hint in STABLE_CORE_HINTS)
        or module.invariant_hits
    ][:24]
...
def refresh_controller_surroundings(
    controller: Any, package_root: Path | None = None
) -> dict[str, Any]:
...
def simulate_virtual_mining_with_hash_landscape(
    controller: Any, proposal: Any
) -> float:
    ...
    horizon = max(
        float(getattr(controller.config, "virtual_session_horizon", 0.25)), 0.01
    )
    ...
    violation_penalty = min(
        0.60, 0.18 * len(getattr(proposal, "constraints_violated", []))
    )
    ...
    quality = max(
        0.0, (1.0 - violation_penalty) * (0.55 * consistency + 0.45 * confidence)
    )

    mining_delta = (landscape_signal - 0.50) * 0.04
    simulated_density = (
        current_density
        + expected_gain * quality
        + mining_delta
...
def bind_runtime_reflexive_adapters(
    controller: Any, package_root: Path | None = None
) -> dict[str, Any]:
