...
from .hendrix_phi_solver import YANG_MILLS_GAP, yang_mills_action
...
def measure_convergence(
    ...
    sampled_errors = [abs(running_means[i-1] - vacuum_energy_target) 
                      for i in sample_indices if i <= len(running_means)]
    
    if len(sampled_errors) >= 3:
        convergence_rate = fit_convergence_rate(sample_indices[:len(sampled_errors)], 
                                                 sampled_errors)
    else:
...
def run_comparative_benchmark(
    ...
    print("=" * 80)
    print()
    print(f"Hypothesis: φ-LCG converges to vacuum energy faster than PRNG")
    ...
    print(f"✓ φ-LCG: {phi_metrics.samples_to_convergence:,} samples, "
    ...
    print(f"✓ PRNG: {prng_metrics.samples_to_convergence:,} samples, "
          f"α={prng_metrics.convergence_rate_exponent:.3f}")
    print()
    
    # Comparative analysis
    convergence_ratio = (phi_metrics.samples_to_convergence / 
                         prng_metrics.samples_to_convergence)
    rate_improvement = (phi_metrics.convergence_rate_exponent / 
                        prng_metrics.convergence_rate_exponent)
    ...
    print(f"  → {'✓ φ-LCG converges FASTER' if convergence_ratio < 1.0 else '✗ PRNG converges faster'}")
    print()
    ...
    print(f"  → Expected MCMC: α ≈ 0.5, QMC conjecture: α ≈ 1.0")
    print()
    print(f"Noise suppression: {noise_suppression_db:.1f} dB")
    print(f"  → {'✓ φ-LCG suppresses variance' if noise_suppression_db > 0 else '✗ PRNG has lower variance'}")
    print()
