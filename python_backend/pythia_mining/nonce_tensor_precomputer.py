...
class NonceTensorPlan:
    ...
    def solver_ranges(self) -> List[Tuple[int, int]]:
        ...
        return [(r.start, r.end) for r in sorted(self.regions, key=lambda x: x.priority, reverse=True)]
    ...
    def precompute(self, target: int, chain_state: Optional[str] = None) -> NonceTensorPlan:
        ...
        for idx in range(num_boundaries):
            ...
            if sv_idx + 1 < len(singular_spectrum):
                mg_align = abs(singular_spectrum[sv_idx + 1] / 
                              max(singular_spectrum[sv_idx], 1e-15) - MASS_GAP_TARGET)
            else:
                mg_align = 1.0
            ...
            regions.append(NonceTensorRegion(
                region_id=idx,
...
def create_nonce_plan_for_mining(target: int, chain_state: Optional[str] = None) -> NonceTensorPlan:
    ...
    print(f"[NonceTensorPlan] Precomputed {len(plan.regions)} nonce regions "
          f"in {plan.precomputation_time_ms:.2f}ms")
    print(f"[NonceTensorPlan] Bond dimension: {plan.bond_dimension} "
          f"(Φ-scaled from {precomputer.bond_dim})")
    print(f"[NonceTensorPlan] Total coverage: {plan.total_nonce_coverage}/{NONCE_SPACE} "
