...
def print_benchmark_comparison(all_results: Dict[str, Dict]):
    ...
    for name, results in all_results.items():
        ...
        print(
            f"{name:<30} {gr['time_to_solution_s']:<15.4f} {gr['success_probability']*100:<15.2f} {speedup:<15.2f}x"
        )
    ...
    for name, results in all_results.items():
        ...
        print(
            f"{name:<30} {qs['execution_time_s']:<15.6f} {qs['fidelity']:<15.4f} {speedup:<15.2f}x"
        )
    ...
    for name, results in all_results.items():
        ...
        speedup = (
            "N/A"
            if not sh["feasible"]
            else f"{baseline_time / sh['execution_time_s']:.2f}x"
        )
        print(
            f"{name:<30} {sh['execution_time_s']:<15.2f} {feasible:<15} {speedup:<15}"
        )
    ...
    for name, results in all_results.items():
        ...
        print(
            f"{name:<30} {vqe['execution_time_s']:<15.4f} {vqe['accuracy']:<15.4f} {speedup:<15.2f}x"
        )
    ...
    for name, results in all_results.items():
        ...
        print(
            f"{name:<30} {qa['execution_time_s']:<15.6f} {qa['approximation_ratio']:<15.4f} {speedup:<15.2f}x"
        )
    ...
    print("-" * 100)
    print(
        f"{'System':<30} {'Physical Error':<20} {'Logical Error':<20} {'Suppression':<15}"
    )
    print("-" * 100)
...
def hardware_scaling_projections():
    ...
    print("-" * 100)
    print(
        f"{'Hardware':<25} {'Cores':<15} {'QOps/sec':<20} {'φ-Speedup':<15} {'Power (W)':<15}"
    )
    print("-" * 100)
    ...
    for hw_name, hw_spec in hardware_configs.items():
        ...
        print(
            f"{hw_name:<25} {hw_spec['cores']:<15} {effective_qops:<20.2e} {phi_speedup:<15.2f}x {hw_spec['power_w']:<15}"
        )
    ...
    print("\n" + "=" * 100)
    print(
        "φ-SCALING ADVANTAGE: HYBA scales linearly with hardware (substrate-agnostic)"
    )
    print(
        "Classical quantum systems require cryogenic infrastructure ($10M+ per system)"
    )
