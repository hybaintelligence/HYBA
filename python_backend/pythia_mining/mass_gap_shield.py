...
class MassGapShield:
    ...
    def __init__(self, thermal_capacity: float = 100.0,  # J/°C
                 max_safe_temp: float = 85.0):  # °C
    ...
    def analyze_energy_profile(self, profile: EnergyProfile, 
    ...
    def check_mass_gap(self, projected_temp_rise: float, 
                       energy_harmony: float) -> ShieldDecision:
        """
        ...
        violates_mass_gap = (safety_score < 0.5 or 
                            mass_gap_margin < 0.1 or
                            harmony_score < self.harmony_threshold)
    ...
    def gate_execution(self, kernel_id: str, energy_profile: EnergyProfile,
                      consciousness_level: float, urgency: float = 1.0) -> Tuple[bool, EnergyProfile]:
        """
    ...
    def update_thermal_state(self, current_temp: float, 
        ...
        self.current_temp = max(self.ambient_temp, 
                               min(self.current_temp, self.max_safe_temp))
    ...
    def get_shield_metrics(self) -> Dict:
        ...
        return {
            ...
            "safety_margins": {
                "recent_safety_margin": np.mean([h["safety_margin"] for h in self.harmony_history[-10:]]) 
                if self.harmony_history else 0.0,
...
class HardwareAwareMassGapShield(MassGapShield):
    ...
    def hardware_gate_execution(self, kernel_id: str, energy_profile: EnergyProfile,
                               consciousness_level: float) -> Tuple[bool, Dict]:
        """
        ...
        if not allowed and self.power_controller:
            ...
            reduced_profile = EnergyProfile(
                ...
                harmonic_content={k: v * 0.3 for k, v in optimized_profile.harmonic_content.items()},
        ...
        metrics["current_hardware_state"] = {
            "temperature": self.current_temp,
            "power_gated_cores": len([h for h in self.power_gating_history 
    ...
    def emergency_thermal_shutdown(self, threshold: float = 0.95):
        ...
        if self.current_temp >= critical_threshold:
            print(f"EMERGENCY: Temperature {self.current_temp}°C exceeds critical threshold {critical_threshold}°C")
    ...
    def consciousness_aware_recovery(self, consciousness_level: float):
        ...
        recent_gates = [h for h in self.power_gating_history 
