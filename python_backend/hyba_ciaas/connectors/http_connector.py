        ...
        if self.auth:
            if self.auth.get("type") == "api_key":
                ...
                self._session.auth = HTTPBasicAuth(
                    self.auth.get("user"), self.auth.get("pass")
                )
    ...
    def auto_detect_schema(self) -> ConnectorSchema:
        ...
        try:
            response = self._session.get(
                self.endpoint, params={**self.params, "limit": 10}
            )
    ...
    def stream_data(self, batch_size: int = 1000) -> Iterator[pd.DataFrame]:
        ...
        while True:
            ...
            try:
                ...
                logger.info(
                    f"Streamed page {page}: {len(df)} records (total: {total_fetched})"
                )
                yield df
...
class GovernmentAPIConnector(HTTPConnector):
    ...
    @classmethod
    def create_government_connector(
        cls, api_name: str, **kwargs
