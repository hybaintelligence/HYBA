...
class ProteinConnector(UniversalConnector):
    ...
    def connect(self):
        ...
        logger.info(
            f"Protein connector initialized (source: {self.source}, embedding: {self.embedding})"
        )
    ...
    def auto_detect_schema(self) -> ConnectorSchema:
        ...
        return ConnectorSchema(
            ...
            * (self.ESM_DIM * 4 + 500),  # ~3.1GB for embeddings
    ...
    def compute_properties(self, sequence: str) -> Dict[str, float]:
        ...
        props = {
            ...
            "charge": sequence.count("K") * 1
            + sequence.count("R") * 1
            - sequence.count("D")
            - sequence.count("E"),
            "hydrophobicity": sum(1 for aa in sequence if aa in "AILMFVP")
            / len(sequence),
            "isoelectric_point": 7.0
            + (sequence.count("K") - sequence.count("D")) * 0.1,
        }
        return props

    def fold_structure(
        self, sequence: str, method: str = "alphafold2"
    ) -> Dict[str, Any]:
        """
