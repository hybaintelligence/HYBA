...
class UniversalConnector(ABC):
    ...
    def normalize_data(
        self, df: pd.DataFrame, config: Optional[NormalizationConfig] = None
    ) -> np.ndarray:
        """
        ...
        if config.feature_engineering and len(numerical_cols) >= 2:
            ...
            for i in range(
                min(len(numerical_cols) - 1, 5)
            ):  # Limit to first 5 to avoid explosion
                for j in range(
                    i + 1, min(i + 3, len(numerical_cols))
                ):  # Limit pairwise combos
    ...
    def ingest_and_normalize(
        self, query: str = None, compression: bool = True
    ) -> Dict[str, Any]:
        """
