...
def test_phi_weights_normalize_and_final_scores_are_bounded(
    scores: list[float],
) -> None:
    ...
    assert all(
        math.isfinite(weight) and weight >= 0.0 for weight in result["phi_weights"]
    )
...
def test_low_variance_cases_use_configured_phi_exponent() -> None:
    ...
    raw = [
        PHI ** (configured_power * (1.0 - abs(score - mean_score))) for score in scores
    ]
...
@given(
    st.dictionaries(
        st.text(min_size=1, max_size=8), finite_strategy, min_size=1, max_size=8
    )
)
@settings(max_examples=100, deadline=None, database=None)
def test_phi_optimized_feature_alignments_stay_bounded(
